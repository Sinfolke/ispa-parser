...
export class LLStringConvertions : public LLInternalData {
    protected:
    std::string convert_var_type(const LLIR::var_types &type, const vector<LLIR::var_type> &data) const;
    ...
    std::string convertExpression(const vector<LLIR::expr> &expression, bool with_braces);
};
