...
namespace IR {
    ...
    using nested_rule_nam = std::vector<std::pair<std::string, std::string>>;
};
void ruleToIr(Parser::Rule &rule_rule, IR::ir &member, int &variable_count, bool isToken, arr_t<std::string> &fullname_arr, IR::var_elements &elements, IR::groups &groups, IR::variables &vars, bool &insideLoop, std::string &fullname, IR::node_ret_t &success_var, char custom_qualifier = -1);
