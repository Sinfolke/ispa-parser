...
public:
    ...
    using ActionTable = utype::unordered_map<std::size_t, utype::unordered_map<stdu::vector<std::string>, Action>>;
    using GotoTable = utype::unordered_map<std::size_t, utype::unordered_map<stdu::vector<std::string>, std::size_t>>;
...
protected:
    ...
    Priority priority;
    Conflicts conflicts;
    // void getPriorityTree(const stdu::vector<stdu::vector<std::string>> *rule, std::unordered_set<stdu::vector<std::string>> &visited, std::size_t depth);
    ...
    auto find_goto_state(const CanonicalItem &item_set, const stdu::vector<std::string> &symbol) -> std::size_t;
    auto find_rules_index(const LR1Core &rule) -> std::size_t;
...
public:
    ...
    auto getGotoTableAsRow() const -> stdu::vector<utype::unordered_map<stdu::vector<std::string>, std::size_t>>;
    // helper functions
    auto getMaxStatesCount() const -> std::size_t;
