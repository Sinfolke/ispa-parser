...
#include <algorithm>
#include <tree.h>
void Tree::normalizeHelper(Parser::arr_t<UniversalRule> &rules, std::vector<std::string> fullname, std::vector<std::vector<std::string>> &nested_rule_names) {
    ...
    for (auto it = rules.begin(); it != rules.end(); it++) {
        auto &el = *it;
        if (el.name == UniversalRules::Rule_group) {
            // Recursively normalize sub-rules
            auto &group_val = Parser::get::Rule_group(el.rule_data);
