...
void DFA::SortedDFA::sort() {
    // Drop const if you intend to actually move data out of min_dfa
    for (auto &state : min_dfa.get()) {
        ...
        for (auto &[key, value] : state.transitions) {
        ...
        states[new_state_id].transitions = std::move(sorted_transitions);
    }
}
