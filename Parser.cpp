#include "Parser.h"
Parser::Rule_res Parser::Parser::accessor_group(Token*& pos) {
	auto in = pos
	::Parser::Token_res _0;
	::Parser::bool_t success_1 = false;
	::Parser::Token_res _2;
	::Parser::bool_t success_3 = false;
	if (!(pos->name == Tokens::AUTO_4))
	{
		return {};
	}
	_0 = *pos;
	success_1 = true;
	pos += 1;
	skipspaces(pos);
	if (!(pos->name == Tokens::NUMBER))
	{
		return {};
	}
	_2 = *pos;
	success_3 = true;
	pos += 1;
	accessor_group_data data = _2;
	return {true, ::Parser::Rule(in->startpos, in->start, pos->end, Rules::accessor_group, data)};
}
Parser::Rule_res Parser::Parser::accessor_element(Token*& pos) {
	auto in = pos;
	::Parser::Token_res _0;
	::Parser::bool_t success_1 = false;
	::Parser::Token_res _2;
	::Parser::bool_t success_3 = false;
	if (!(pos->name == Tokens::AUTO_1))
	{
		return {};
	}
	_0 = *pos;
	success_1 = true;
	pos += 1;
	skipspaces(pos);
	if (!(pos->name == Tokens::NUMBER))
	{
		return {};
	}
	_2 = *pos;
	success_3 = true;
	pos += 1;
	accessor_element_data data = _2;
	return {true, ::Parser::Rule(in->startpos, in->start, pos->end, Rules::accessor_element, data)};
}
Parser::Rule_res Parser::Parser::accessor_char(Token*& pos) {
	auto in = pos;
	::Parser::Token_res _0;
	::Parser::bool_t success_1 = false;
	::Parser::Token_res _2;
	::Parser::bool_t success_3 = false;
	if (!(pos->name == Tokens::AUTO_91))
	{
		return {};
	}
	_0 = *pos;
	success_1 = true;
	pos += 1;
	skipspaces(pos);
	if (!(pos->name == Tokens::NUMBER))
	{
		return {};
	}
	_2 = *pos;
	success_3 = true;
	pos += 1;
	accessor_char_data data = _2;
	return {true, ::Parser::Rule(in->startpos, in->start, pos->end, Rules::accessor_char, data)};
}
Parser::Rule_res Parser::Parser::accessor_all(Token*& pos) {
	auto in = pos;
	::Parser::Rule _0;
	::Parser::bool_t success_1 = false;
	::Parser::Rule_res _2;
	::Parser::bool_t success_3 = false;
	::Parser::Rule_res _4;
	::Parser::bool_t success_5 = false;
	::Parser::Rule_res _6;
	::Parser::bool_t success_7 = false;
	_2 = accessor_group(pos);
	if (!(_2.result))
	{
		_4 = accessor_element(pos);
		if (!(_4.result))
		{
			_6 = accessor_char(pos);
			if (!(_6.result))
			{
				return {};
			}
			else 
			{
				success_7 = true;
				_0 = _6.token;
			}
		}
		else 
		{
			success_5 = true;
			_0 = _4.token;
		}
	}
	else 
	{
		success_3 = true;
		_0 = _2.token;
	}
	_0 += ::Parser::str_t(pos, 0);
	success_1 = true;
	pos += 0;
	accessor_all_data data = _0;
	return {true, ::Parser::Rule(in->startpos, in->start, pos->end, Rules::accessor_all, data)};
}
Parser::Rule_res Parser::Parser::accessor(Token*& pos) {
	auto in = pos;
	::Parser::Rule_res _0;
	::Parser::bool_t success_1 = false;
	::Parser::arr_t<::Parser::str_t> shadow_11;
	::Parser::str_t _2;
	::Parser::bool_t success_3 = false;
	::Parser::Token_res _4;
	::Parser::bool_t success_5 = false;
	::Parser::arr_t<::Parser::Token_res> shadow_6;
	::Parser::Rule_res _7;
	::Parser::bool_t success_8 = false;
	::Parser::arr_t<::Parser::Rule_res> shadow_9;
	_0 = accessor_all(pos);
	if (!(_0.result))
	{
		return {};
	}
	success_1 = true;
	pos += 0;
	skipspaces(pos);
	auto begin_10 = pos;
	while (1)
	{
		if (!(begin_10->name == Tokens::AUTO_3))
		{
			break;
		}
		_4 = *begin_10;
		success_5 = true;
		begin_10 += 1;
		shadow_6.push_back(_4);
		skipspaces(pos);
		_7 = accessor_all(begin_10);
		if (!(_7.result))
		{
			break;
		}
		success_8 = true;
		begin_10 += 0;
		shadow_9.push_back(NONE);
		shadow_11.push_back(_2);
	}
	if (success_8)
	{
		success_3 = true;
		pos = begin_10;
	}
	_2 += shadow_6;
	_2 += shadow_9;
	accessor_data data;
	data.second = shadow_9;
	data.first = _0.token;

	return {true, ::Parser::Rule(in->startpos, in->start, pos->end, Rules::accessor, data)};
}
Parser::Rule_res Parser::Parser::cll(Token*& pos) {
	auto in = pos;
	::Parser::Token cll_begin;
	::Parser::bool_t success_0 = false;
	::Parser::Token_res _1;
	::Parser::bool_t success_2 = false;
	::Parser::arr_t<::Parser::str_t> shadow_37;
	::Parser::str_t _4;
	::Parser::bool_t success_5 = false;
	::Parser::bool_t success_38 = false;
	::Parser::arr_t<::Parser::Rule> shadow_32;
	::Parser::Rule _6;
	::Parser::bool_t success_7 = false;
	::Parser::Rule _8;
	::Parser::bool_t success_9 = false;
	::Parser::Rule_res _10;
	::Parser::bool_t success_11 = false;
	::Parser::arr_t<::Parser::Rule_res> shadow_12;
	::Parser::Rule_res _13;
	::Parser::bool_t success_14 = false;
	::Parser::arr_t<::Parser::Rule_res> shadow_15;
	::Parser::Rule_res _16;
	::Parser::bool_t success_17 = false;
	::Parser::arr_t<::Parser::Rule_res> shadow_18;
	::Parser::Rule_res _19;
	::Parser::bool_t success_20 = false;
	::Parser::arr_t<::Parser::Rule_res> shadow_21;
	::Parser::Rule_res _22;
	::Parser::bool_t success_23 = false;
	::Parser::arr_t<::Parser::Rule_res> shadow_24;
	::Parser::Rule_res _25;
	::Parser::bool_t success_26 = false;
	::Parser::arr_t<::Parser::Rule_res> shadow_27;
	::Parser::Rule_res _28;
	::Parser::bool_t success_29 = false;
	::Parser::arr_t<::Parser::Rule_res> shadow_30;
	::Parser::Token_res _33;
	::Parser::bool_t success_34 = false;
	::Parser::arr_t<::Parser::Token_res> shadow_35;
	auto begin_3 = pos;
	if (!(begin_3->name == Tokens::AUTO_4))
	{
		return {};
	}
	_1 = *begin_3;
	success_2 = true;
	begin_3 += 1;
	if (success_2)
	{
		success_0 = true;
		pos = begin_3;
	}
	cll_begin = _1;
	skipspaces(pos);
	auto begin_36 = pos;
	while (1)
	{
		auto begin_31 = begin_36;
		while (1)
		{
			_10 = cll_var(begin_31);
			if (!(_10.result))
			{
				_13 = cll_if(begin_31);
				if (!(_13.result))
				{
					_16 = expr(begin_31);
					if (!(_16.result))
					{
						_19 = copiable_method_call(begin_31);
						if (!(_19.result))
						{
							_22 = method_call(begin_31);
							if (!(_22.result))
							{
								_25 = loop_while(begin_31);
								if (!(_25.result))
								{
									_28 = loop_for(begin_31);
									if (!(_28.result))
									{
										break;
									}
									else 
									{
										success_29 = true;
										shadow_30.push_back(NONE);
										_8 = shadow_30;
									}
								}
								else 
								{
									success_26 = true;
									shadow_27.push_back(NONE);
									_8 = shadow_27;
								}
							}
							else 
							{
								success_23 = true;
								shadow_24.push_back(NONE);
								_8 = shadow_24;
							}
						}
						else 
						{
							success_20 = true;
							shadow_21.push_back(NONE);
							_8 = shadow_21;
						}
					}
					else 
					{
						success_17 = true;
						shadow_18.push_back(NONE);
						_8 = shadow_18;
					}
				}
				else 
				{
					success_14 = true;
					shadow_15.push_back(NONE);
					_8 = shadow_15;
				}
			}
			else 
			{
				success_11 = true;
				shadow_12.push_back(NONE);
				_8 = shadow_12;
			}
			_8 += ::Parser::str_t(begin_31, 0);
			success_9 = true;
			begin_31 += 0;
			shadow_32.push_back(_6);
		}
		if (success_9)
		{
			success_7 = true;
			begin_36 = begin_31;
		}
		_6 = _8;
		skipspaces(pos);
		if (!(begin_36->name == Tokens::END))
		{
			break;
		}
		_33 = *begin_36;
		success_34 = true;
		begin_36 += 1;
		shadow_35.push_back(_33);
		shadow_37.push_back(_4);
		success_38 = true;
	}
	if (!success_38)
	{
		return {};
	}
	if (success_34)
	{
		success_5 = true;
		pos = begin_36;
	}
	_4 += shadow_32;
	_4 += shadow_35;
	cll_data data = shadow_37;
	return {true, ::Parser::Rule(in->startpos, in->start, pos->end, Rules::cll, data)};
}
Parser::Rule_res Parser::Parser::cll_block(Token*& pos) {
	auto in = pos;
	::Parser::Token_res _0;
	::Parser::bool_t success_1 = false;
	::Parser::Rule_res _2;
	::Parser::bool_t success_3 = false;
	::Parser::Token_res _4;
	::Parser::bool_t success_5 = false;
	if (!(pos->name == Tokens::AUTO_65))
	{
		return {};
	}
	_0 = *pos;
	success_1 = true;
	pos += 1;
	skipspaces(pos);
	_2 = Rule_rule(pos);
	while (_2.result)
	{
		success_3 = true;
		pos += 0;
		_2 = Rule_rule(pos);
	}
	skipspaces(pos);
	if (!(pos->name == Tokens::AUTO_70))
	{
		return {};
	}
	_4 = *pos;
	success_5 = true;
	pos += 1;
	cll_block_data data = _2.token;
	return {true, ::Parser::Rule(in->startpos, in->start, pos->end, Rules::cll_block, data)};
}
Parser::Rule_res Parser::Parser::cll_spaced_block(Token*& pos) {
	auto in = pos;
	::Parser::Rule_res _0;
	::Parser::bool_t success_1 = false;
	::Parser::arr_t<::Parser::Rule_res> shadow_2;
	while ((skipup(" ")>spaces_amount))
	{
		_0 = Rule_rule(pos);
		if (!(_0.result))
		{
			return {};
		}
		success_1 = true;
		pos += 0;
		shadow_2.push_back(NONE);
		skipspaces(pos);
	}
	cll_spaced_block_data data = shadow_2;
	return {true, ::Parser::Rule(in->startpos, in->start, pos->end, Rules::cll_spaced_block, data)};
}
Parser::Token_res Parser::Tokenizator::OP(const char* &pos) {
	auto in = pos;
	::Parser::any_t _0;
	::Parser::bool_t success_1 = false;
	::Parser::str_t _2;
	::Parser::bool_t success_3 = false;
	if (!(*(pos + 0) == '+'))
	{
		::Parser::str_t _4;
		::Parser::bool_t success_5 = false;
		if (!(*(pos + 1) == '-'))
		{
			::Parser::str_t _6;
			::Parser::bool_t success_7 = false;
			if (!(*(pos + 2) == '*'))
			{
				::Parser::str_t _8;
				::Parser::bool_t success_9 = false;
				if (!(*(pos + 3) == '/'))
				{
					::Parser::str_t _10;
					::Parser::bool_t success_11 = false;
					if (!(*(pos + 4) == '%'))
					{
					}
					else 
					{
						_10 += ::Parser::str_t(pos, 5);
						success_11 = true;
						_0 = _10;
					}
				}
				else 
				{
					_8 += ::Parser::str_t(pos, 5);
					success_9 = true;
					_0 = _8;
				}
			}
			else 
			{
				_6 += ::Parser::str_t(pos, 5);
				success_7 = true;
				_0 = _6;
			}
		}
		else 
		{
			_4 += ::Parser::str_t(pos, 5);
			success_5 = true;
			_0 = _4;
		}
	}
	else 
	{
		_2 += ::Parser::str_t(pos, 5);
		success_3 = true;
		_0 = _2;
	}
	_0 += ::Parser::str_t(pos, 5);
	success_1 = true;
	pos += 5;
	skipspaces(pos);
	::Parser::str_t _12;
	::Parser::bool_t success_13 = false;
	::Parser::str_t _14;
	::Parser::bool_t success_15 = false;
	if (!(*(pos + 0) == '&'))
	{
		::Parser::str_t _16;
		::Parser::bool_t success_17 = false;
		if (!(*(pos + 1) == '|'))
		{
			::Parser::str_t _18;
			::Parser::bool_t success_19 = false;
			if (!(*(pos + 2) == '^'))
			{
				::Parser::str_t _20;
				::Parser::bool_t success_21 = false;
				if (!(!std::strncmp(pos, "<<", 2)))
				{
					::Parser::str_t _22;
					::Parser::bool_t success_23 = false;
					if (!(!std::strncmp(pos, ">>", 2)))
					{
						return {};
					}
					else 
					{
						_22 += ::Parser::str_t(pos, 7);
						success_23 = true;
						_12 = _22;
					}
				}
				else 
				{
					_20 += ::Parser::str_t(pos, 7);
					success_21 = true;
					_12 = _20;
				}
			}
			else 
			{
				_18 += ::Parser::str_t(pos, 7);
				success_19 = true;
				_12 = _18;
			}
		}
		else 
		{
			_16 += ::Parser::str_t(pos, 7);
			success_17 = true;
			_12 = _16;
		}
	}
	else 
	{
		_14 += ::Parser::str_t(pos, 7);
		success_15 = true;
		_12 = _14;
	}
	_12 += ::Parser::str_t(pos, 7);
	success_13 = true;
	pos += 7;
	OP_data data = _0;
	return {true, ::Parser::Token(in - str, in, pos, Tokens::OP, data)};
}
Parser::Token_res Parser::Tokenizator::ASSIGNMENT_OP(const char* &pos) {
	auto in = pos;
	::Parser::Token_res _0;
	::Parser::bool_t success_1 = false;
	_0 = OP(pos);
	if (_0.result)
	{
		success_1 = true;
		pos += 0;
	}
	skipspaces(pos);
	::Parser::Token_res _2;
	::Parser::bool_t success_3 = false;
	_2 = AUTO_55(pos);
	if (!(_2.result))
	{
		return {};
	}
	success_3 = true;
	pos += 0;
	ASSIGNMENT_OP_data data = _0.token;
	return {true, ::Parser::Token(in - str, in, pos, Tokens::ASSIGNMENT_OP, data)};
}
Parser::Token_res Parser::Tokenizator::COMPARE_OP(const char* &pos) {
	auto in = pos;
	::Parser::str_t _0;
	::Parser::bool_t success_1 = false;
	::Parser::str_t _2;
	::Parser::bool_t success_3 = false;
	if (!(!std::strncmp(pos, "==", 2)))
	{
		::Parser::str_t _4;
		::Parser::bool_t success_5 = false;
		if (!(!std::strncmp(pos, "!=", 2)))
		{
			::Parser::str_t _6;
			::Parser::bool_t success_7 = false;
			if (!(*(pos + 4) == '>'))
			{
				::Parser::str_t _8;
				::Parser::bool_t success_9 = false;
				if (!(*(pos + 5) == '<'))
				{
					::Parser::str_t _10;
					::Parser::bool_t success_11 = false;
					if (!(!std::strncmp(pos, ">=", 2)))
					{
						::Parser::str_t _12;
						::Parser::bool_t success_13 = false;
						if (!(!std::strncmp(pos, "<=", 2)))
						{
							return {};
						}
						else 
						{
							_12 += ::Parser::str_t(pos, 10);
							success_13 = true;
							_0 = _12;
						}
					}
					else 
					{
						_10 += ::Parser::str_t(pos, 10);
						success_11 = true;
						_0 = _10;
					}
				}
				else 
				{
					_8 += ::Parser::str_t(pos, 10);
					success_9 = true;
					_0 = _8;
				}
			}
			else 
			{
				_6 += ::Parser::str_t(pos, 10);
				success_7 = true;
				_0 = _6;
			}
		}
		else 
		{
			_4 += ::Parser::str_t(pos, 10);
			success_5 = true;
			_0 = _4;
		}
	}
	else 
	{
		_2 += ::Parser::str_t(pos, 10);
		success_3 = true;
		_0 = _2;
	}
	_0 += ::Parser::str_t(pos, 10);
	success_1 = true;
	pos += 10;
	COMPARE_OP_data data = _0;
	return {true, ::Parser::Token(in - str, in, pos, Tokens::COMPARE_OP, data)};
}
Parser::Token_res Parser::Tokenizator::LOGICAL_OP(const char* &pos) {
	auto in = pos;
	::Parser::Token _0;
	::Parser::bool_t success_1 = false;
	::Parser::Token_res _2;
	::Parser::bool_t success_3 = false;
	_2 = LOGICAL_AND(pos);
	if (!(_2.result))
	{
		::Parser::Token_res _4;
		::Parser::bool_t success_5 = false;
		_4 = LOGICAL_OR(pos);
		if (!(_4.result))
		{
			return {};
		}
		else 
		{
			success_5 = true;
			_0 = _4.token;
		}
	}
	else 
	{
		success_3 = true;
		_0 = _2.token;
	}
	_0 += ::Parser::str_t(pos, 0);
	success_1 = true;
	pos += 0;
	LOGICAL_OP_data data = _0;
	return {true, ::Parser::Token(in - str, in, pos, Tokens::LOGICAL_OP, data)};
}
Parser::Token_res Parser::Tokenizator::LOGICAL_NOT(const char* &pos) {
	auto in = pos;
	::Parser::str_t _0;
	::Parser::bool_t success_1 = false;
	::Parser::str_t _2;
	::Parser::bool_t success_3 = false;
	if (!(*(pos + 0) == '!'))
	{
		::Parser::str_t _4;
		::Parser::bool_t success_5 = false;
		if (!(!std::strncmp(pos, "not", 3)))
		{
			return {};
		}
		else 
		{
			_4 += ::Parser::str_t(pos, 4);
			success_5 = true;
			_0 = _4;
		}
	}
	else 
	{
		_2 += ::Parser::str_t(pos, 4);
		success_3 = true;
		_0 = _2;
	}
	_0 += ::Parser::str_t(pos, 4);
	success_1 = true;
	pos += 4;
	LOGICAL_NOT_data data = _0;
	return {true, ::Parser::Token(in - str, in, pos, Tokens::LOGICAL_NOT, data)};
}
Parser::Token_res Parser::Tokenizator::LOGICAL_AND(const char* &pos) {
	auto in = pos;
	::Parser::str_t _0;
	::Parser::bool_t success_1 = false;
	::Parser::str_t _2;
	::Parser::bool_t success_3 = false;
	if (!(!std::strncmp(pos, "&&", 2)))
	{
		::Parser::str_t _4;
		::Parser::bool_t success_5 = false;
		if (!(!std::strncmp(pos, "and", 3)))
		{
			return {};
		}
		else 
		{
			_4 += ::Parser::str_t(pos, 5);
			success_5 = true;
			_0 = _4;
		}
	}
	else 
	{
		_2 += ::Parser::str_t(pos, 5);
		success_3 = true;
		_0 = _2;
	}
	_0 += ::Parser::str_t(pos, 5);
	success_1 = true;
	pos += 5;
	LOGICAL_AND_data data = _0;
	return {true, ::Parser::Token(in - str, in, pos, Tokens::LOGICAL_AND, data)};
}
Parser::Token_res Parser::Tokenizator::LOGICAL_OR(const char* &pos) {
	auto in = pos;
	::Parser::str_t _0;
	::Parser::bool_t success_1 = false;
	::Parser::str_t _2;
	::Parser::bool_t success_3 = false;
	if (!(!std::strncmp(pos, "||", 2)))
	{
		::Parser::str_t _4;
		::Parser::bool_t success_5 = false;
		if (!(!std::strncmp(pos, "or", 2)))
		{
			return {};
		}
		else 
		{
			_4 += ::Parser::str_t(pos, 4);
			success_5 = true;
			_0 = _4;
		}
	}
	else 
	{
		_2 += ::Parser::str_t(pos, 4);
		success_3 = true;
		_0 = _2;
	}
	_0 += ::Parser::str_t(pos, 4);
	success_1 = true;
	pos += 4;
	LOGICAL_OR_data data = _0;
	return {true, ::Parser::Token(in - str, in, pos, Tokens::LOGICAL_OR, data)};
}
Parser::Token_res Parser::Tokenizator::LOGICAL_ANDR(const char* &pos) {
	auto in = pos;
	::Parser::any_t _0;
	::Parser::bool_t success_1 = false;
	::Parser::str_t _2;
	::Parser::bool_t success_3 = false;
	if (!(!std::strncmp(pos, "|&", 2)))
	{
		::Parser::str_t _4;
		::Parser::bool_t success_5 = false;
		auto begin_13 = pos;
		do
		{
			::Parser::str_t _6;
			::Parser::bool_t success_7 = false;
			if (!(!std::strncmp(pos, "and", 3)))
			{
				break;
			}
			_6 += ::Parser::str_t(begin_13, 5);
			success_7 = true;
			begin_13 += 5;
			::Parser::arr_t<::Parser::str_t> shadow_10;
			::Parser::str_t _8;
			::Parser::bool_t success_9 = false;
			if (*(pos + 0) != ' ')
			{
				break;
				shadow_10.push_back(_8);
			}
			_8 += ::Parser::str_t(begin_13, 1);
			success_9 = true;
			begin_13 += 1;
			::Parser::str_t _11;
			::Parser::bool_t success_12 = false;
			if (!(!std::strncmp(pos, "or", 2)))
			{
				break;
			}
			_11 += ::Parser::str_t(begin_13, 2);
			success_12 = true;
			begin_13 += 2;
		}		while(0)
;
		if (success_12)
		{
			success_5 = true;
			pos = begin_13;
		}
		_4 += _6;
		_4 += shadow_10;
		_4 += _11;
		if (!success_5)
		{
			return {};
		}
		else 
		{
			_0 = _4;
		}
	}
	else 
	{
		_2 += ::Parser::str_t(pos, 0);
		success_3 = true;
		_0 = _2;
	}
	_0 += ::Parser::str_t(pos, 0);
	success_1 = true;
	pos += 0;
	LOGICAL_ANDR_data data = _6;
	return {true, ::Parser::Token(in - str, in, pos, Tokens::LOGICAL_ANDR, data)};
}
Parser::Rule_res Parser::Parser::cll_template_content_typename(Token*& pos) {
	auto in = pos;
	::Parser::Rule_res _0;
	::Parser::bool_t success_1 = false;
	::Parser::arr_t<::Parser::str_t> shadow_11;
	::Parser::str_t _2;
	::Parser::bool_t success_3 = false;
	::Parser::Token_res _4;
	::Parser::bool_t success_5 = false;
	::Parser::arr_t<::Parser::Token_res> shadow_6;
	::Parser::Rule_res _7;
	::Parser::bool_t success_8 = false;
	::Parser::arr_t<::Parser::Rule_res> shadow_9;
	_0 = cll_type(pos);
	if (!(_0.result))
	{
		return {};
	}
	success_1 = true;
	pos += 0;
	skipspaces(pos);
	auto begin_10 = pos;
	while (1)
	{
		if (!(begin_10->name == Tokens::AUTO_78))
		{
			break;
		}
		_4 = *begin_10;
		success_5 = true;
		begin_10 += 1;
		shadow_6.push_back(_4);
		skipspaces(pos);
		_7 = cll_type(begin_10);
		if (!(_7.result))
		{
			break;
		}
		success_8 = true;
		begin_10 += 0;
		shadow_9.push_back(NONE);
		shadow_11.push_back(_2);
	}
	if (success_8)
	{
		success_3 = true;
		pos = begin_10;
	}
	_2 += shadow_6;
	_2 += shadow_9;
	cll_template_content_typename_data data = {,};
	return {true, ::Parser::Rule(in->startpos, in->start, pos->end, Rules::cll_template_content_typename, data)};
}
Parser::Rule_res Parser::Parser::cll_template_content_int(Token*& pos) {
	auto in = pos;
	::Parser::Token_res _0;
	::Parser::bool_t success_1 = false;
	::Parser::arr_t<::Parser::str_t> shadow_11;
	::Parser::str_t _2;
	::Parser::bool_t success_3 = false;
	::Parser::Token_res _4;
	::Parser::bool_t success_5 = false;
	::Parser::arr_t<::Parser::Token_res> shadow_6;
	::Parser::Token_res _7;
	::Parser::bool_t success_8 = false;
	::Parser::arr_t<::Parser::Token_res> shadow_9;
	if (!(pos->name == Tokens::NUMBER))
	{
		return {};
	}
	_0 = *pos;
	success_1 = true;
	pos += 1;
	skipspaces(pos);
	auto begin_10 = pos;
	while (1)
	{
		if (!(begin_10->name == Tokens::AUTO_78))
		{
			break;
		}
		_4 = *begin_10;
		success_5 = true;
		begin_10 += 1;
		shadow_6.push_back(_4);
		skipspaces(pos);
		if (!(begin_10->name == Tokens::NUMBER))
		{
			break;
		}
		_7 = *begin_10;
		success_8 = true;
		begin_10 += 1;
		shadow_9.push_back(_7);
		shadow_11.push_back(_2);
	}
	if (success_8)
	{
		success_3 = true;
		pos = begin_10;
	}
	_2 += shadow_6;
	_2 += shadow_9;
	cll_template_content_int_data data = {,};
	return {true, ::Parser::Rule(in->startpos, in->start, pos->end, Rules::cll_template_content_int, data)};
}
Parser::Rule_res Parser::Parser::cll_template_content_bool(Token*& pos) {
	auto in = pos;
	::Parser::Token_res _0;
	::Parser::bool_t success_1 = false;
	::Parser::arr_t<::Parser::str_t> shadow_11;
	::Parser::str_t _2;
	::Parser::bool_t success_3 = false;
	::Parser::Token_res _4;
	::Parser::bool_t success_5 = false;
	::Parser::arr_t<::Parser::Token_res> shadow_6;
	::Parser::Token_res _7;
	::Parser::bool_t success_8 = false;
	::Parser::arr_t<::Parser::Token_res> shadow_9;
	if (!(pos->name == Tokens::BOOLEAN))
	{
		return {};
	}
	_0 = *pos;
	success_1 = true;
	pos += 1;
	skipspaces(pos);
	auto begin_10 = pos;
	while (1)
	{
		if (!(begin_10->name == Tokens::AUTO_78))
		{
			break;
		}
		_4 = *begin_10;
		success_5 = true;
		begin_10 += 1;
		shadow_6.push_back(_4);
		skipspaces(pos);
		if (!(begin_10->name == Tokens::BOOLEAN))
		{
			break;
		}
		_7 = *begin_10;
		success_8 = true;
		begin_10 += 1;
		shadow_9.push_back(_7);
		shadow_11.push_back(_2);
	}
	if (success_8)
	{
		success_3 = true;
		pos = begin_10;
	}
	_2 += shadow_6;
	_2 += shadow_9;
	cll_template_content_bool_data data = {,};
	return {true, ::Parser::Rule(in->startpos, in->start, pos->end, Rules::cll_template_content_bool, data)};
}
Parser::Rule_res Parser::Parser::cll_template_content_str(Token*& pos) {
	auto in = pos;
	::Parser::Token_res _0;
	::Parser::bool_t success_1 = false;
	::Parser::arr_t<::Parser::str_t> shadow_11;
	::Parser::str_t _2;
	::Parser::bool_t success_3 = false;
	::Parser::Token_res _4;
	::Parser::bool_t success_5 = false;
	::Parser::arr_t<::Parser::Token_res> shadow_6;
	::Parser::Token_res _7;
	::Parser::bool_t success_8 = false;
	::Parser::arr_t<::Parser::Token_res> shadow_9;
	if (!(pos->name == Tokens::STRING))
	{
		return {};
	}
	_0 = *pos;
	success_1 = true;
	pos += 1;
	skipspaces(pos);
	auto begin_10 = pos;
	while (1)
	{
		if (!(begin_10->name == Tokens::AUTO_78))
		{
			break;
		}
		_4 = *begin_10;
		success_5 = true;
		begin_10 += 1;
		shadow_6.push_back(_4);
		skipspaces(pos);
		if (!(begin_10->name == Tokens::STRING))
		{
			break;
		}
		_7 = *begin_10;
		success_8 = true;
		begin_10 += 1;
		shadow_9.push_back(_7);
		shadow_11.push_back(_2);
	}
	if (success_8)
	{
		success_3 = true;
		pos = begin_10;
	}
	_2 += shadow_6;
	_2 += shadow_9;
	cll_template_content_str_data data = {,};
	return {true, ::Parser::Rule(in->startpos, in->start, pos->end, Rules::cll_template_content_str, data)};
}
Parser::Rule_res Parser::Parser::cll_template_content_arr(Token*& pos) {
	auto in = pos;
	::Parser::Rule_res _0;
	::Parser::bool_t success_1 = false;
	::Parser::arr_t<::Parser::str_t> shadow_11;
	::Parser::str_t _2;
	::Parser::bool_t success_3 = false;
	::Parser::Token_res _4;
	::Parser::bool_t success_5 = false;
	::Parser::arr_t<::Parser::Token_res> shadow_6;
	::Parser::Rule_res _7;
	::Parser::bool_t success_8 = false;
	::Parser::arr_t<::Parser::Rule_res> shadow_9;
	_0 = array(pos);
	if (!(_0.result))
	{
		return {};
	}
	success_1 = true;
	pos += 0;
	skipspaces(pos);
	auto begin_10 = pos;
	while (1)
	{
		if (!(begin_10->name == Tokens::AUTO_78))
		{
			break;
		}
		_4 = *begin_10;
		success_5 = true;
		begin_10 += 1;
		shadow_6.push_back(_4);
		skipspaces(pos);
		_7 = array(begin_10);
		if (!(_7.result))
		{
			break;
		}
		success_8 = true;
		begin_10 += 0;
		shadow_9.push_back(NONE);
		shadow_11.push_back(_2);
	}
	if (success_8)
	{
		success_3 = true;
		pos = begin_10;
	}
	_2 += shadow_6;
	_2 += shadow_9;
	cll_template_content_arr_data data = {,};
	return {true, ::Parser::Rule(in->startpos, in->start, pos->end, Rules::cll_template_content_arr, data)};
}
Parser::Rule_res Parser::Parser::cll_template_content_obj(Token*& pos) {
	auto in = pos;
	::Parser::Rule_res _0;
	::Parser::bool_t success_1 = false;
	::Parser::arr_t<::Parser::str_t> shadow_11;
	::Parser::str_t _2;
	::Parser::bool_t success_3 = false;
	::Parser::Token_res _4;
	::Parser::bool_t success_5 = false;
	::Parser::arr_t<::Parser::Token_res> shadow_6;
	::Parser::Rule_res _7;
	::Parser::bool_t success_8 = false;
	::Parser::arr_t<::Parser::Rule_res> shadow_9;
	_0 = object(pos);
	if (!(_0.result))
	{
		return {};
	}
	success_1 = true;
	pos += 0;
	skipspaces(pos);
	auto begin_10 = pos;
	while (1)
	{
		if (!(begin_10->name == Tokens::AUTO_78))
		{
			break;
		}
		_4 = *begin_10;
		success_5 = true;
		begin_10 += 1;
		shadow_6.push_back(_4);
		skipspaces(pos);
		_7 = object(begin_10);
		if (!(_7.result))
		{
			break;
		}
		success_8 = true;
		begin_10 += 0;
		shadow_9.push_back(NONE);
		shadow_11.push_back(_2);
	}
	if (success_8)
	{
		success_3 = true;
		pos = begin_10;
	}
	_2 += shadow_6;
	_2 += shadow_9;
	cll_template_content_obj_data data = {,};
	return {true, ::Parser::Rule(in->startpos, in->start, pos->end, Rules::cll_template_content_obj, data)};
}
Parser::Rule_res Parser::Parser::cll_template_content_any_data(Token*& pos) {
	auto in = pos;
	::Parser::Rule_res _0;
	::Parser::bool_t success_1 = false;
	::Parser::arr_t<::Parser::str_t> shadow_11;
	::Parser::str_t _2;
	::Parser::bool_t success_3 = false;
	::Parser::Token_res _4;
	::Parser::bool_t success_5 = false;
	::Parser::arr_t<::Parser::Token_res> shadow_6;
	::Parser::Rule_res _7;
	::Parser::bool_t success_8 = false;
	::Parser::arr_t<::Parser::Rule_res> shadow_9;
	_0 = any_data(pos);
	if (!(_0.result))
	{
		return {};
	}
	success_1 = true;
	pos += 0;
	skipspaces(pos);
	auto begin_10 = pos;
	while (1)
	{
		if (!(begin_10->name == Tokens::AUTO_78))
		{
			break;
		}
		_4 = *begin_10;
		success_5 = true;
		begin_10 += 1;
		shadow_6.push_back(_4);
		skipspaces(pos);
		_7 = any_data(begin_10);
		if (!(_7.result))
		{
			break;
		}
		success_8 = true;
		begin_10 += 0;
		shadow_9.push_back(NONE);
		shadow_11.push_back(_2);
	}
	if (success_8)
	{
		success_3 = true;
		pos = begin_10;
	}
	_2 += shadow_6;
	_2 += shadow_9;
	cll_template_content_any_data_data data = {,};
	return {true, ::Parser::Rule(in->startpos, in->start, pos->end, Rules::cll_template_content_any_data, data)};
}
Parser::Rule_res Parser::Parser::cll_csupport_types(Token*& pos) {
	auto in = pos;
	::Parser::bool_t is_unsigned = matched();
	::Parser::str_t _0;
	::Parser::bool_t success_1 = false;
	::Parser::str_t _2;
	::Parser::bool_t success_3 = false;
	::Parser::Token_res _4;
	::Parser::bool_t success_5 = false;
	::Parser::Token _6;
	::Parser::bool_t success_7 = false;
	::Parser::Token _8;
	::Parser::bool_t success_9 = false;
	::Parser::Token_res _10;
	::Parser::bool_t success_11 = false;
	::Parser::Token_res _12;
	::Parser::bool_t success_13 = false;
	::Parser::Token_res _14;
	::Parser::bool_t success_15 = false;
	::Parser::Token_res _16;
	::Parser::bool_t success_17 = false;
	::Parser::Token_res _18;
	::Parser::bool_t success_19 = false;
	::Parser::Token _22;
	::Parser::bool_t success_23 = false;
	::Parser::Token _24;
	::Parser::bool_t success_25 = false;
	::Parser::Token_res _26;
	::Parser::bool_t success_27 = false;
	::Parser::Token_res _28;
	::Parser::bool_t success_29 = false;
	::Parser::Token _31;
	::Parser::bool_t success_32 = false;
	::Parser::Token _33;
	::Parser::bool_t success_34 = false;
	::Parser::Token_res _35;
	::Parser::bool_t success_36 = false;
	::Parser::Token_res _37;
	::Parser::bool_t success_38 = false;
	::Parser::Token_res _39;
	::Parser::bool_t success_40 = false;
	::Parser::Token_res _41;
	::Parser::bool_t success_42 = false;
	::Parser::Token_res _43;
	::Parser::bool_t success_44 = false;
	::Parser::Token_res _45;
	::Parser::bool_t success_46 = false;
	::Parser::Rule_res _48;
	::Parser::bool_t success_49 = false;
	auto begin_21 = pos;
	do
	{
		if (begin_21->name == Tokens::AUTO_14)
		{
			_4 = *begin_21;
			success_5 = true;
			begin_21 += 1;
		}
		skipspaces(pos);
		auto begin_20 = begin_21;
		if (!(begin_20->name == Tokens::AUTO_15))
		{
			if (!(begin_20->name == Tokens::AUTO_16))
			{
				if (!(begin_20->name == Tokens::AUTO_17))
				{
					if (!(begin_20->name == Tokens::AUTO_18))
					{
						if (!(begin_20->name == Tokens::AUTO_19))
						{
							break;
						}
						else 
						{
							_18 = *begin_20;
							success_19 = true;
							_8 = _18;
						}
					}
					else 
					{
						_16 = *begin_20;
						success_17 = true;
						_8 = _16;
					}
				}
				else 
				{
					_14 = *begin_20;
					success_15 = true;
					_8 = _14;
				}
			}
			else 
			{
				_12 = *begin_20;
				success_13 = true;
				_8 = _12;
			}
		}
		else 
		{
			_10 = *begin_20;
			success_11 = true;
			_8 = _10;
		}
		_8 += ::Parser::str_t(begin_20, 5);
		success_9 = true;
		begin_20 += 5;
		if (success_9)
		{
			success_7 = true;
			begin_21 = begin_20;
		}
		_6 = _8;
	}	while(0)
;
	if (success_7)
	{
		success_3 = true;
		pos = begin_21;
	}
	_2 += _4;
	_2 += _6;
	if (!success_3)
	{
		auto begin_30 = pos;
		do
		{
			if (!(begin_30->name == Tokens::AUTO_20))
			{
				if (!(begin_30->name == Tokens::AUTO_21))
				{
					break;
				}
				else 
				{
					_28 = *begin_30;
					success_29 = true;
					_24 = _28;
				}
			}
			else 
			{
				_26 = *begin_30;
				success_27 = true;
				_24 = _26;
			}
			_24 += ::Parser::str_t(begin_30, 2);
			success_25 = true;
			begin_30 += 2;
		}		while(0)
;
		if (success_25)
		{
			success_23 = true;
			pos = begin_30;
		}
		_22 = _24;
		if (!success_23)
		{
			auto begin_47 = pos;
			do
			{
				if (!(begin_47->name == Tokens::AUTO_22))
				{
					if (!(begin_47->name == Tokens::AUTO_23))
					{
						if (!(begin_47->name == Tokens::AUTO_24))
						{
							if (!(begin_47->name == Tokens::AUTO_25))
							{
								if (!(begin_47->name == Tokens::AUTO_26))
								{
									if (!(begin_47->name == Tokens::AUTO_27))
									{
										break;
									}
									else 
									{
										_45 = *begin_47;
										success_46 = true;
										_33 = _45;
									}
								}
								else 
								{
									_43 = *begin_47;
									success_44 = true;
									_33 = _43;
								}
							}
							else 
							{
								_41 = *begin_47;
								success_42 = true;
								_33 = _41;
							}
						}
						else 
						{
							_39 = *begin_47;
							success_40 = true;
							_33 = _39;
						}
					}
					else 
					{
						_37 = *begin_47;
						success_38 = true;
						_33 = _37;
					}
				}
				else 
				{
					_35 = *begin_47;
					success_36 = true;
					_33 = _35;
				}
				_33 += ::Parser::str_t(begin_47, 6);
				success_34 = true;
				begin_47 += 6;
			}			while(0)
;
			if (success_34)
			{
				success_32 = true;
				pos = begin_47;
			}
			_31 = _33;
			if (!success_32)
			{
				return {};
			}
			else 
			{
				_0 = _31;
			}
		}
		else 
		{
			_0 = _22;
		}
	}
	else 
	{
		_0 = _2;
	}
	_0 += ::Parser::str_t(pos, 0);
	success_1 = true;
	pos += 0;
	skipspaces(pos);
	_48 = cll_template_typename(pos);
	if (!(_48.result))
	{
		return {};
	}
	success_49 = true;
	pos += 0;
	cll_csupport_types_data data;
	data.template = _8;
	data.type = _4;

	return {true, ::Parser::Rule(in->startpos, in->start, pos->end, Rules::cll_csupport_types, data)};
}
Parser::Rule_res Parser::Parser::cll_type_abstract(Token*& pos) {
	auto in = pos;
	::Parser::any_t _0;
	::Parser::bool_t success_1 = false;
	::Parser::any_t _2;
	::Parser::bool_t success_3 = false;
	::Parser::Token_res _4;
	::Parser::bool_t success_5 = false;
	::Parser::Token_res _6;
	::Parser::bool_t success_7 = false;
	::Parser::Token_res _8;
	::Parser::bool_t success_9 = false;
	::Parser::Token_res _10;
	::Parser::bool_t success_11 = false;
	::Parser::Token_res _12;
	::Parser::bool_t success_13 = false;
	::Parser::str_t _14;
	::Parser::bool_t success_15 = false;
	::Parser::Token_res _16;
	::Parser::bool_t success_17 = false;
	::Parser::Rule_res _18;
	::Parser::bool_t success_19 = false;
	::Parser::str_t _21;
	::Parser::bool_t success_22 = false;
	::Parser::Token_res _23;
	::Parser::bool_t success_24 = false;
	::Parser::Rule_res _25;
	::Parser::bool_t success_26 = false;
	auto begin_28 = pos;
	if (!(begin_28->name == Tokens::AUTO_28))
	{
		if (!(begin_28->name == Tokens::AUTO_29))
		{
			if (!(begin_28->name == Tokens::AUTO_30))
			{
				if (!(begin_28->name == Tokens::AUTO_31))
				{
					if (!(begin_28->name == Tokens::AUTO_32))
					{
						auto begin_20 = begin_28;
						do
						{
							if (!(begin_20->name == Tokens::AUTO_33))
							{
								break;
							}
							_16 = *begin_20;
							success_17 = true;
							begin_20 += 6;
							skipspaces(pos);
							_18 = cll_template_typename(begin_20);
							if (!(_18.result))
							{
								break;
							}
							success_19 = true;
							begin_20 += 0;
						}						while(0)
;
						if (success_19)
						{
							success_15 = true;
							begin_28 = begin_20;
						}
						_14 += _16;
						_14 += _18.token;
						if (!success_15)
						{
							auto begin_27 = begin_28;
							do
							{
								if (!(begin_27->name == Tokens::AUTO_34))
								{
									break;
								}
								_23 = *begin_27;
								success_24 = true;
								begin_27 += 1;
								skipspaces(pos);
								_25 = cll_template_typename(begin_27);
								if (!(_25.result))
								{
									break;
								}
								success_26 = true;
								begin_27 += 0;
							}							while(0)
;
							if (success_26)
							{
								success_22 = true;
								begin_28 = begin_27;
							}
							_21 += _23;
							_21 += _25.token;
							if (!success_22)
							{
								return {};
							}
							else 
							{
								_2 = _21;
							}
						}
						else 
						{
							_2 = _14;
						}
					}
					else 
					{
						_12 = *begin_28;
						success_13 = true;
						_2 = _12;
					}
				}
				else 
				{
					_10 = *begin_28;
					success_11 = true;
					_2 = _10;
				}
			}
			else 
			{
				_8 = *begin_28;
				success_9 = true;
				_2 = _8;
			}
		}
		else 
		{
			_6 = *begin_28;
			success_7 = true;
			_2 = _6;
		}
	}
	else 
	{
		_4 = *begin_28;
		success_5 = true;
		_2 = _4;
	}
	_2 += ::Parser::str_t(begin_28, 0);
	success_3 = true;
	begin_28 += 0;
	if (success_3)
	{
		success_1 = true;
		pos = begin_28;
	}
	cll_type_abstract_data data;
	data.template = _23;
	data.type = _16;

	return {true, ::Parser::Rule(in->startpos, in->start, pos->end, Rules::cll_type_abstract, data)};
}
Parser::Rule_res Parser::Parser::cll_type(Token*& pos) {
	auto in = pos;
	::Parser::Rule _0;
	::Parser::bool_t success_1 = false;
	::Parser::Rule_res _2;
	::Parser::bool_t success_3 = false;
	::Parser::Rule_res _4;
	::Parser::bool_t success_5 = false;
	_2 = cll_csupport_types(pos);
	if (!(_2.result))
	{
		_4 = cll_type_abstract(pos);
		if (!(_4.result))
		{
			return {};
		}
		else 
		{
			success_5 = true;
			_0 = _4.token;
		}
	}
	else 
	{
		success_3 = true;
		_0 = _2.token;
	}
	_0 += ::Parser::str_t(pos, 0);
	success_1 = true;
	pos += 0;
	cll_type_data data = _0;
	return {true, ::Parser::Rule(in->startpos, in->start, pos->end, Rules::cll_type, data)};
}
Parser::Rule_res Parser::Parser::cll_if(Token*& pos) {
	auto in = pos;
	::Parser::Token _0;
	::Parser::bool_t success_1 = false;
	::Parser::Token_res _2;
	::Parser::bool_t success_3 = false;
	::Parser::arr_t<::Parser::Token_res> shadow_4;
	::Parser::Token_res _6;
	::Parser::bool_t success_7 = false;
	::Parser::Rule_res _8;
	::Parser::bool_t success_9 = false;
	::Parser::str_t _10;
	::Parser::bool_t success_11 = false;
	::Parser::Rule _12;
	::Parser::bool_t success_13 = false;
	::Parser::Rule_res _14;
	::Parser::bool_t success_15 = false;
	::Parser::Rule_res _16;
	::Parser::bool_t success_17 = false;
	::Parser::str_t _18;
	::Parser::bool_t success_19 = false;
	::Parser::UNDEF _21;
	::Parser::UNDEF shadow_22;
	::Parser::bool_t success_20 = false;
	::Parser::Token_res _23;
	::Parser::bool_t success_24 = false;
	::Parser::Rule_res _25;
	::Parser::bool_t success_26 = false;
	::Parser::UNDEF _27;
	::Parser::bool_t success_28 = false;
	auto begin_5 = pos;
	while (begin_5->name == Tokens::AUTO_35)
	{
		_2 += *begin_5;
		success_3 = true;
		begin_5 += 1;
		shadow_4.push_back(_2);
	}
	if (success_3)
	{
		success_1 = true;
		pos = begin_5;
	}
	_0 = _2;
	skipspaces(pos);
	if (!(pos->name == Tokens::AUTO_36))
	{
		return {};
	}
	_6 = *pos;
	success_7 = true;
	pos += 1;
	skipspaces(pos);
	_8 = expr(pos);
	if (!(_8.result))
	{
		return {};
	}
	success_9 = true;
	pos += 0;
	skipspaces(pos);
	auto begin_31 = pos;
	_14 = cll_block(begin_31);
	if (!(_14.result))
	{
		_16 = cll_spaced_block(begin_31);
		if (!(_16.result))
		{
			return {};
		}
		else 
		{
			success_17 = true;
			_12 = _16.token;
		}
	}
	else 
	{
		success_15 = true;
		_12 = _14.token;
	}
	_12 += ::Parser::str_t(begin_31, 0);
	success_13 = true;
	begin_31 += 0;
	skipspaces(pos);
	auto begin_30 = begin_31;
	if (!(!std::strncmp(pos, shadow_4, strlen(shadow_4))))
	{
		return {};
	}
	_21 += ::Parser::str_t(begin_30, 0 + strlen(shadow_4));
	success_20 = true;
	begin_30 += 0;
	skipspaces(pos);
	if (!(begin_30->name == Tokens::AUTO_107))
	{
		return {};
	}
	_23 = *begin_30;
	success_24 = true;
	begin_30 += 1;
	skipspaces(pos);
	_25 = size(begin_30);
	if (!(_25.result))
	{
		return {};
	}
	success_26 = true;
	begin_30 += 0;
	skipspaces(pos);
	auto begin_29 = begin_30;
	if ()
	{
		success_28 = true;
		begin_30 = begin_29;
	}
	if (success_28)
	{
		success_19 = true;
		begin_31 = begin_30;
	}
	_18 += shadow_22;
	_18 += _23;
	_18 += _25.token;
	_18 += _27;
	if (success_19)
	{
		success_11 = true;
		pos = begin_31;
	}
	_10 += _12;
	_10 += _18;
	cll_if_data data;
	data.block = _0;
	data.expr = _6;

	return {true, ::Parser::Rule(in->startpos, in->start, pos->end, Rules::cll_if, data)};
}
Parser::Rule_res Parser::Parser::cll_ternary(Token*& pos) {
	auto in = pos;
	::Parser::Rule_res _0;
	::Parser::bool_t success_1 = false;
	::Parser::Token_res _2;
	::Parser::bool_t success_3 = false;
	::Parser::Rule_res _4;
	::Parser::bool_t success_5 = false;
	::Parser::Token_res _6;
	::Parser::bool_t success_7 = false;
	::Parser::Rule_res _8;
	::Parser::bool_t success_9 = false;
	_0 = expr(pos);
	if (!(_0.result))
	{
		return {};
	}
	success_1 = true;
	pos += 0;
	skipspaces(pos);
	if (!(pos->name == Tokens::AUTO_84))
	{
		return {};
	}
	_2 = *pos;
	success_3 = true;
	pos += 1;
	skipspaces(pos);
	_4 = expr(pos);
	if (!(_4.result))
	{
		return {};
	}
	success_5 = true;
	pos += 0;
	skipspaces(pos);
	if (!(pos->name == Tokens::AUTO_105))
	{
		return {};
	}
	_6 = *pos;
	success_7 = true;
	pos += 1;
	skipspaces(pos);
	_8 = expr(pos);
	if (!(_8.result))
	{
		return {};
	}
	success_9 = true;
	pos += 0;
	cll_ternary_data data;
	data.second = _8.token;
	data.first = _4.token;
	data.cond = _0.token;

	return {true, ::Parser::Rule(in->startpos, in->start, pos->end, Rules::cll_ternary, data)};
}
Parser::Rule_res Parser::Parser::expr_logical(Token*& pos) {
	auto in = pos;
	::Parser::Rule _0;
	::Parser::bool_t success_1 = false;
	::Parser::Rule_res _2;
	::Parser::bool_t success_3 = false;
	::Parser::Rule_res _4;
	::Parser::bool_t success_5 = false;
	::Parser::Rule_res _6;
	::Parser::bool_t success_7 = false;
	::Parser::Token_res _8;
	::Parser::bool_t success_9 = false;
	::Parser::Rule _10;
	::Parser::bool_t success_11 = false;
	::Parser::Rule_res _12;
	::Parser::bool_t success_13 = false;
	::Parser::Rule_res _14;
	::Parser::bool_t success_15 = false;
	::Parser::Rule_res _16;
	::Parser::bool_t success_17 = false;
	_2 = expr_compare(pos);
	if (!(_2.result))
	{
		_4 = expr_arithmetic(pos);
		if (!(_4.result))
		{
			_6 = expr_for_arithmetic(pos);
			if (!(_6.result))
			{
				return {};
			}
			else 
			{
				success_7 = true;
				_0 = _6.token;
			}
		}
		else 
		{
			success_5 = true;
			_0 = _4.token;
		}
	}
	else 
	{
		success_3 = true;
		_0 = _2.token;
	}
	_0 += ::Parser::str_t(pos, 0);
	success_1 = true;
	pos += 0;
	skipspaces(pos);
	if (!(pos->name == Tokens::LOGICAL_OP))
	{
		return {};
	}
	_8 = *pos;
	success_9 = true;
	pos += 1;
	skipspaces(pos);
	_12 = expr_compare(pos);
	if (!(_12.result))
	{
		_14 = expr_arithmetic(pos);
		if (!(_14.result))
		{
			_16 = expr_for_arithmetic(pos);
			if (!(_16.result))
			{
				return {};
			}
			else 
			{
				success_17 = true;
				_10 = _16.token;
			}
		}
		else 
		{
			success_15 = true;
			_10 = _14.token;
		}
	}
	else 
	{
		success_13 = true;
		_10 = _12.token;
	}
	_10 += ::Parser::str_t(pos, 0);
	success_11 = true;
	pos += 0;
	expr_logical_data data;
	data.right = _10;
	data.op = _8;
	data.left = _0;

	return {true, ::Parser::Rule(in->startpos, in->start, pos->end, Rules::expr_logical, data)};
}
Parser::Rule_res Parser::Parser::expr_compare(Token*& pos) {
	auto in = pos;
	::Parser::Rule_res _0;
	::Parser::bool_t success_1 = false;
	::Parser::arr_t<::Parser::str_t> shadow_11;
	::Parser::str_t _2;
	::Parser::bool_t success_3 = false;
	::Parser::bool_t success_12 = false;
	::Parser::Token_res _4;
	::Parser::bool_t success_5 = false;
	::Parser::arr_t<::Parser::Token_res> shadow_6;
	::Parser::Rule_res _7;
	::Parser::bool_t success_8 = false;
	::Parser::arr_t<::Parser::Rule_res> shadow_9;
	_0 = expr_arithmetic(pos);
	if (!(_0.result))
	{
		return {};
	}
	success_1 = true;
	pos += 0;
	skipspaces(pos);
	auto begin_10 = pos;
	while (1)
	{
		if (!(begin_10->name == Tokens::COMPARE_OP))
		{
			break;
		}
		_4 = *begin_10;
		success_5 = true;
		begin_10 += 1;
		shadow_6.push_back(_4);
		skipspaces(pos);
		_7 = expr_arithmetic(begin_10);
		if (!(_7.result))
		{
			break;
		}
		success_8 = true;
		begin_10 += 0;
		shadow_9.push_back(NONE);
		shadow_11.push_back(_2);
		success_12 = true;
	}
	if (!success_12)
	{
		return {};
	}
	if (success_8)
	{
		success_3 = true;
		pos = begin_10;
	}
	_2 += shadow_6;
	_2 += shadow_9;
	expr_compare_data data;
	data.sequence = shadow_9;
	data.operators = shadow_6;
	data.first = _0.token;

	return {true, ::Parser::Rule(in->startpos, in->start, pos->end, Rules::expr_compare, data)};
}
Parser::Rule_res Parser::Parser::expr_arithmetic(Token*& pos) {
	auto in = pos;
	::Parser::Rule_res _0;
	::Parser::bool_t success_1 = false;
	::Parser::arr_t<::Parser::str_t> shadow_11;
	::Parser::str_t _2;
	::Parser::bool_t success_3 = false;
	::Parser::bool_t success_12 = false;
	::Parser::Token_res _4;
	::Parser::bool_t success_5 = false;
	::Parser::arr_t<::Parser::Token_res> shadow_6;
	::Parser::Rule_res _7;
	::Parser::bool_t success_8 = false;
	::Parser::arr_t<::Parser::Rule_res> shadow_9;
	_0 = expr_for_arithmetic(pos);
	if (!(_0.result))
	{
		return {};
	}
	success_1 = true;
	pos += 0;
	skipspaces(pos);
	auto begin_10 = pos;
	while (1)
	{
		if (!(begin_10->name == Tokens::OP))
		{
			break;
		}
		_4 = *begin_10;
		success_5 = true;
		begin_10 += 1;
		shadow_6.push_back(_4);
		skipspaces(pos);
		_7 = expr_for_arithmetic(begin_10);
		if (!(_7.result))
		{
			break;
		}
		success_8 = true;
		begin_10 += 0;
		shadow_9.push_back(NONE);
		shadow_11.push_back(_2);
		success_12 = true;
	}
	if (!success_12)
	{
		return {};
	}
	if (success_8)
	{
		success_3 = true;
		pos = begin_10;
	}
	_2 += shadow_6;
	_2 += shadow_9;
	expr_arithmetic_data data;
	data.sequence = shadow_9;
	data.operators = shadow_6;
	data.first = _0.token;

	return {true, ::Parser::Rule(in->startpos, in->start, pos->end, Rules::expr_arithmetic, data)};
}
Parser::Rule_res Parser::Parser::expr_for_arithmetic(Token*& pos) {
	auto in = pos;
	::Parser::Rule _0;
	::Parser::bool_t success_1 = false;
	::Parser::Rule_res _2;
	::Parser::bool_t success_3 = false;
	::Parser::Rule_res _4;
	::Parser::bool_t success_5 = false;
	::Parser::Rule_res _6;
	::Parser::bool_t success_7 = false;
	::Parser::Rule_res _8;
	::Parser::bool_t success_9 = false;
	_2 = expr_group(pos);
	if (!(_2.result))
	{
		_4 = method_call(pos);
		if (!(_4.result))
		{
			_6 = cll_function_call(pos);
			if (!(_6.result))
			{
				_8 = any_data(pos);
				if (!(_8.result))
				{
					return {};
				}
				else 
				{
					success_9 = true;
					_0 = _8.token;
				}
			}
			else 
			{
				success_7 = true;
				_0 = _6.token;
			}
		}
		else 
		{
			success_5 = true;
			_0 = _4.token;
		}
	}
	else 
	{
		success_3 = true;
		_0 = _2.token;
	}
	_0 += ::Parser::str_t(pos, 0);
	success_1 = true;
	pos += 0;
	expr_for_arithmetic_data data = _0;
	return {true, ::Parser::Rule(in->startpos, in->start, pos->end, Rules::expr_for_arithmetic, data)};
}
Parser::Rule_res Parser::Parser::expr_group(Token*& pos) {
	auto in = pos;
	::Parser::Token_res _0;
	::Parser::bool_t success_1 = false;
	::Parser::Rule _2;
	::Parser::bool_t success_3 = false;
	::Parser::Rule_res _4;
	::Parser::bool_t success_5 = false;
	::Parser::Rule_res _6;
	::Parser::bool_t success_7 = false;
	::Parser::Token_res _8;
	::Parser::bool_t success_9 = false;
	if (!(pos->name == Tokens::AUTO_40))
	{
		return {};
	}
	_0 = *pos;
	success_1 = true;
	pos += 1;
	skipspaces(pos);
	_4 = cll_ternary(pos);
	if (!(_4.result))
	{
		_6 = expr(pos);
		if (!(_6.result))
		{
			return {};
		}
		else 
		{
			success_7 = true;
			_2 = _6.token;
		}
	}
	else 
	{
		success_5 = true;
		_2 = _4.token;
	}
	_2 += ::Parser::str_t(pos, 0);
	success_3 = true;
	pos += 0;
	skipspaces(pos);
	if (!(pos->name == Tokens::AUTO_41))
	{
		return {};
	}
	_8 = *pos;
	success_9 = true;
	pos += 1;
	expr_group_data data = _2;
	return {true, ::Parser::Rule(in->startpos, in->start, pos->end, Rules::expr_group, data)};
}
Parser::Rule_res Parser::Parser::expr_expr_copiable_method_call(Token*& pos) {
	auto in = pos;
	::Parser::Token_res _0;
	::Parser::bool_t success_1 = false;
	::Parser::Rule_res _2;
	::Parser::bool_t success_3 = false;
	::Parser::Token_res _4;
	::Parser::bool_t success_5 = false;
	if (!(pos->name == Tokens::AUTO_42))
	{
		return {};
	}
	_0 = *pos;
	success_1 = true;
	pos += 1;
	skipspaces(pos);
	_2 = copiable_method_call(pos);
	if (!(_2.result))
	{
		return {};
	}
	success_3 = true;
	pos += 0;
	skipspaces(pos);
	if (!(pos->name == Tokens::AUTO_43))
	{
		return {};
	}
	_4 = *pos;
	success_5 = true;
	pos += 1;
	expr_expr_copiable_method_call_data data = _2.token;
	return {true, ::Parser::Rule(in->startpos, in->start, pos->end, Rules::expr_expr_copiable_method_call, data)};
}
Parser::Rule_res Parser::Parser::expr(Token*& pos) {
	auto in = pos;
	::Parser::Rule _0;
	::Parser::bool_t success_1 = false;
	::Parser::Rule_res _2;
	::Parser::bool_t success_3 = false;
	::Parser::Rule_res _4;
	::Parser::bool_t success_5 = false;
	::Parser::Rule_res _6;
	::Parser::bool_t success_7 = false;
	::Parser::Rule_res _8;
	::Parser::bool_t success_9 = false;
	::Parser::Rule_res _10;
	::Parser::bool_t success_11 = false;
	::Parser::Rule_res _12;
	::Parser::bool_t success_13 = false;
	::Parser::Rule_res _14;
	::Parser::bool_t success_15 = false;
	_2 = expr_logical(pos);
	if (!(_2.result))
	{
		_4 = expr_compare(pos);
		if (!(_4.result))
		{
			_6 = expr_arithmetic(pos);
			if (!(_6.result))
			{
				_8 = expr_group(pos);
				if (!(_8.result))
				{
					_10 = expr_expr_copiable_method_call(pos);
					if (!(_10.result))
					{
						_12 = cll_function_call(pos);
						if (!(_12.result))
						{
							_14 = any_data(pos);
							if (!(_14.result))
							{
								return {};
							}
							else 
							{
								success_15 = true;
								_0 = _14.token;
							}
						}
						else 
						{
							success_13 = true;
							_0 = _12.token;
						}
					}
					else 
					{
						success_11 = true;
						_0 = _10.token;
					}
				}
				else 
				{
					success_9 = true;
					_0 = _8.token;
				}
			}
			else 
			{
				success_7 = true;
				_0 = _6.token;
			}
		}
		else 
		{
			success_5 = true;
			_0 = _4.token;
		}
	}
	else 
	{
		success_3 = true;
		_0 = _2.token;
	}
	_0 += ::Parser::str_t(pos, 0);
	success_1 = true;
	pos += 0;
	expr_data data = _0;
	return {true, ::Parser::Rule(in->startpos, in->start, pos->end, Rules::expr, data)};
}
Parser::Rule_res Parser::Parser::function_body_call(Token*& pos) {
	auto in = pos;
	::Parser::Token_res _0;
	::Parser::bool_t success_1 = false;
	::Parser::Rule_res _2;
	::Parser::bool_t success_3 = false;
	::Parser::Token_res _4;
	::Parser::bool_t success_5 = false;
	if (!(pos->name == Tokens::AUTO_88))
	{
		return {};
	}
	_0 = *pos;
	success_1 = true;
	pos += 1;
	skipspaces(pos);
	_2 = function_arguments(pos);
	if (!(_2.result))
	{
		return {};
	}
	success_3 = true;
	pos += 0;
	skipspaces(pos);
	if (!(pos->name == Tokens::AUTO_89))
	{
		return {};
	}
	_4 = *pos;
	success_5 = true;
	pos += 1;
	function_body_call_data data = _2.token;
	return {true, ::Parser::Rule(in->startpos, in->start, pos->end, Rules::function_body_call, data)};
}
Parser::Rule_res Parser::Parser::function_body_decl(Token*& pos) {
	auto in = pos;
	::Parser::Token_res _0;
	::Parser::bool_t success_1 = false;
	::Parser::Rule_res _2;
	::Parser::bool_t success_3 = false;
	::Parser::Token_res _4;
	::Parser::bool_t success_5 = false;
	if (!(pos->name == Tokens::AUTO_88))
	{
		return {};
	}
	_0 = *pos;
	success_1 = true;
	pos += 1;
	skipspaces(pos);
	_2 = function_parameters(pos);
	if (!(_2.result))
	{
		return {};
	}
	success_3 = true;
	pos += 0;
	skipspaces(pos);
	if (!(pos->name == Tokens::AUTO_89))
	{
		return {};
	}
	_4 = *pos;
	success_5 = true;
	pos += 1;
	function_body_decl_data data = _2.token;
	return {true, ::Parser::Rule(in->startpos, in->start, pos->end, Rules::function_body_decl, data)};
}
Parser::Rule_res Parser::Parser::function_arguments(Token*& pos) {
	auto in = pos;
	::Parser::any_t _0;
	::Parser::bool_t success_1 = false;
	::Parser::any_t _2;
	::Parser::bool_t success_3 = false;
	::Parser::Rule_res _4;
	::Parser::bool_t success_5 = false;
	::Parser::Token_res _6;
	::Parser::bool_t success_7 = false;
	::Parser::arr_t<::Parser::str_t> shadow_23;
	::Parser::str_t _9;
	::Parser::bool_t success_10 = false;
	::Parser::str_t _11;
	::Parser::bool_t success_12 = false;
	::Parser::arr_t<::Parser::str_t> shadow_13;
	::Parser::any_t _14;
	::Parser::bool_t success_15 = false;
	::Parser::Rule_res _16;
	::Parser::bool_t success_17 = false;
	::Parser::arr_t<::Parser::Rule_res> shadow_18;
	::Parser::Token_res _19;
	::Parser::bool_t success_20 = false;
	::Parser::arr_t<::Parser::Token_res> shadow_21;
	auto begin_8 = pos;
	do
	{
		_4 = any_data(begin_8);
		if (!(_4.result))
		{
			if (!(begin_8->name == Tokens::ID))
			{
				break;
			}
			else 
			{
				_6 = *begin_8;
				success_7 = true;
				_2 = _6;
			}
		}
		else 
		{
			success_5 = true;
			_2 = _4.token;
		}
		_2 += ::Parser::str_t(begin_8, 1);
		success_3 = true;
		begin_8 += 1;
	}	while(0)
;
	if (success_3)
	{
		success_1 = true;
		pos = begin_8;
	}
	skipspaces(pos);
	if ((matched()))
	{
		auto begin_22 = pos;
		while (1)
		{
			if (!(*(pos + 0) == ','))
			{
				break;
			}
			_11 += ::Parser::str_t(begin_22, 1);
			success_12 = true;
			begin_22 += 1;
			shadow_13.push_back(_11);
			skipspaces(pos);
			_16 = any_data(begin_22);
			if (!(_16.result))
			{
				if (!(begin_22->name == Tokens::ID))
				{
					break;
				}
				else 
				{
					_19 = *begin_22;
					success_20 = true;
					shadow_21.push_back(_19);
					_14 = shadow_21;
				}
			}
			else 
			{
				success_17 = true;
				shadow_18.push_back(NONE);
				_14 = shadow_18;
			}
			_14 += ::Parser::str_t(begin_22, 1);
			success_15 = true;
			begin_22 += 1;
			shadow_23.push_back(_9);
		}
		if (success_15)
		{
			success_10 = true;
			pos = begin_22;
		}
		_9 += shadow_13;
		_9 += _14;
		skipspaces(pos);
	}
	function_arguments_data data = {,};
	return {true, ::Parser::Rule(in->startpos, in->start, pos->end, Rules::function_arguments, data)};
}
Parser::Rule_res Parser::Parser::function_parameters(Token*& pos) {
	auto in = pos;
	::Parser::Token_res _0;
	::Parser::bool_t success_1 = false;
	::Parser::arr_t<::Parser::str_t> shadow_11;
	::Parser::str_t _2;
	::Parser::bool_t success_3 = false;
	::Parser::str_t _4;
	::Parser::bool_t success_5 = false;
	::Parser::arr_t<::Parser::str_t> shadow_6;
	::Parser::Token_res _7;
	::Parser::bool_t success_8 = false;
	::Parser::arr_t<::Parser::Token_res> shadow_9;
	if (pos->name == Tokens::ID)
	{
		_0 = *pos;
		success_1 = true;
		pos += 1;
	}
	skipspaces(pos);
	if ((matched()))
	{
		auto begin_10 = pos;
		while (1)
		{
			if (!(*(pos + 0) == ','))
			{
				break;
			}
			_4 += ::Parser::str_t(begin_10, 1);
			success_5 = true;
			begin_10 += 1;
			shadow_6.push_back(_4);
			skipspaces(pos);
			if (!(begin_10->name == Tokens::ID))
			{
				break;
			}
			_7 = *begin_10;
			success_8 = true;
			begin_10 += 1;
			shadow_9.push_back(_7);
			shadow_11.push_back(_2);
		}
		if (success_8)
		{
			success_3 = true;
			pos = begin_10;
		}
		_2 += shadow_6;
		_2 += shadow_9;
		skipspaces(pos);
	}
	function_parameters_data data = {,};
	return {true, ::Parser::Rule(in->startpos, in->start, pos->end, Rules::function_parameters, data)};
}
Parser::Rule_res Parser::Parser::cll_function_call(Token*& pos) {
	auto in = pos;
	::Parser::Token_res _0;
	::Parser::bool_t success_1 = false;
	::Parser::Rule_res _2;
	::Parser::bool_t success_3 = false;
	if (!(pos->name == Tokens::ID))
	{
		return {};
	}
	_0 = *pos;
	success_1 = true;
	pos += 1;
	skipspaces(pos);
	_2 = function_body_call(pos);
	if (!(_2.result))
	{
		return {};
	}
	success_3 = true;
	pos += 0;
	cll_function_call_data data;
	data.body = _2.token;
	data.name = _0;

	return {true, ::Parser::Rule(in->startpos, in->start, pos->end, Rules::cll_function_call, data)};
}
Parser::Rule_res Parser::Parser::function_decl(Token*& pos) {
	auto in = pos;
	::Parser::Token_res _0;
	::Parser::bool_t success_1 = false;
	::Parser::Token _2;
	::Parser::bool_t success_3 = false;
	::Parser::Token_res _4;
	::Parser::bool_t success_5 = false;
	::Parser::arr_t<::Parser::Token_res> shadow_6;
	::Parser::bool_t success_7 = false;
	::Parser::Rule_res _9;
	::Parser::bool_t success_10 = false;
	::Parser::Rule val;
	::Parser::bool_t success_11 = false;
	::Parser::Rule _12;
	::Parser::bool_t success_13 = false;
	::Parser::Rule_res _14;
	::Parser::bool_t success_15 = false;
	::Parser::Rule_res _16;
	::Parser::bool_t success_17 = false;
	if (!(pos->name == Tokens::AUTO_48))
	{
		return {};
	}
	_0 = *pos;
	success_1 = true;
	pos += 1;
	skipspaces(pos);
	auto begin_8 = pos;
	while (begin_8->name == Tokens::ID)
	{
		_4 += *begin_8;
		success_5 = true;
		begin_8 += 1;
		shadow_6.push_back(_4);
		success_7 = true;
	}
	if (!success_7)
	{
		return {};
	}
	if (success_5)
	{
		success_3 = true;
		pos = begin_8;
	}
	_2 = _4;
	skipspaces(pos);
	_9 = function_body_decl(pos);
	if (!(_9.result))
	{
		return {};
	}
	success_10 = true;
	pos += 0;
	skipspaces(pos);
	auto begin_18 = pos;
	do
	{
		_14 = cll_block(begin_18);
		if (!(_14.result))
		{
			_16 = cll_spaced_block(begin_18);
			if (!(_16.result))
			{
				break;
			}
			else 
			{
				success_17 = true;
				_12 = _16.token;
			}
		}
		else 
		{
			success_15 = true;
			_12 = _14.token;
		}
		_12 += ::Parser::str_t(begin_18, 0);
		success_13 = true;
		begin_18 += 0;
	}	while(0)
;
	if (success_13)
	{
		success_11 = true;
		pos = begin_18;
	}
	val = _12;
	function_decl_data data;
	data.value = val;
	data.name = _2;
	data.type = shadow_6;

	return {true, ::Parser::Rule(in->startpos, in->start, pos->end, Rules::function_decl, data)};
}
Parser::Rule_res Parser::Parser::loop_while(Token*& pos) {
	auto in = pos;
	::Parser::Token_res _0;
	::Parser::bool_t success_1 = false;
	::Parser::Rule_res _2;
	::Parser::bool_t success_3 = false;
	::Parser::Rule _4;
	::Parser::bool_t success_5 = false;
	::Parser::Rule_res _6;
	::Parser::bool_t success_7 = false;
	::Parser::Rule_res _8;
	::Parser::bool_t success_9 = false;
	if (!(pos->name == Tokens::AUTO_49))
	{
		return {};
	}
	_0 = *pos;
	success_1 = true;
	pos += 1;
	skipspaces(pos);
	_2 = expr(pos);
	if (!(_2.result))
	{
		return {};
	}
	success_3 = true;
	pos += 0;
	skipspaces(pos);
	_6 = cll_block(pos);
	if (!(_6.result))
	{
		_8 = cll_spaced_block(pos);
		if (!(_8.result))
		{
			return {};
		}
		else 
		{
			success_9 = true;
			_4 = _8.token;
		}
	}
	else 
	{
		success_7 = true;
		_4 = _6.token;
	}
	_4 += ::Parser::str_t(pos, 0);
	success_5 = true;
	pos += 0;
	loop_while_data data;
	data.block = _4;
	data.expr = _2.token;

	return {true, ::Parser::Rule(in->startpos, in->start, pos->end, Rules::loop_while, data)};
}
Parser::Rule_res Parser::Parser::loop_for(Token*& pos) {
	auto in = pos;
	::Parser::Rule_res _0;
	::Parser::bool_t success_1 = false;
	::Parser::Token_res _2;
	::Parser::bool_t success_3 = false;
	::Parser::Rule _4;
	::Parser::bool_t success_5 = false;
	::Parser::Rule _6;
	::Parser::bool_t success_7 = false;
	::Parser::Rule_res _8;
	::Parser::bool_t success_9 = false;
	::Parser::Rule_res _10;
	::Parser::bool_t success_11 = false;
	::Parser::Token_res _13;
	::Parser::bool_t success_14 = false;
	::Parser::Rule_res _15;
	::Parser::bool_t success_16 = false;
	::Parser::Token_res _17;
	::Parser::bool_t success_18 = false;
	::Parser::Rule_res _19;
	::Parser::bool_t success_20 = false;
	::Parser::Token_res _21;
	::Parser::bool_t success_22 = false;
	::Parser::Rule _23;
	::Parser::bool_t success_24 = false;
	::Parser::Rule_res _25;
	::Parser::bool_t success_26 = false;
	::Parser::Rule_res _27;
	::Parser::bool_t success_28 = false;
	_0 = for(pos);
	if (!(_0.result))
	{
		return {};
	}
	success_1 = true;
	pos += 0;
	skipspaces(pos);
	if (!(pos->name == Tokens::AUTO_88))
	{
		return {};
	}
	_2 = *pos;
	success_3 = true;
	pos += 1;
	skipspaces(pos);
	auto begin_12 = pos;
	do
	{
		_8 = expr(begin_12);
		if (!(_8.result))
		{
			_10 = cll_var(begin_12);
			if (!(_10.result))
			{
				break;
			}
			else 
			{
				success_11 = true;
				_6 = _10.token;
			}
		}
		else 
		{
			success_9 = true;
			_6 = _8.token;
		}
		_6 += ::Parser::str_t(begin_12, 0);
		success_7 = true;
		begin_12 += 0;
	}	while(0)
;
	if (success_7)
	{
		success_5 = true;
		pos = begin_12;
	}
	_4 = _6;
	skipspaces(pos);
	if (!(pos->name == Tokens::AUTO_103))
	{
		return {};
	}
	_13 = *pos;
	success_14 = true;
	pos += 1;
	skipspaces(pos);
	_15 = expr(pos);
	if (_15.result)
	{
		success_16 = true;
		pos += 0;
	}
	skipspaces(pos);
	if (!(pos->name == Tokens::AUTO_103))
	{
		return {};
	}
	_17 = *pos;
	success_18 = true;
	pos += 1;
	skipspaces(pos);
	_19 = expr(pos);
	if (_19.result)
	{
		success_20 = true;
		pos += 0;
	}
	skipspaces(pos);
	if (!(pos->name == Tokens::AUTO_89))
	{
		return {};
	}
	_21 = *pos;
	success_22 = true;
	pos += 1;
	skipspaces(pos);
	_25 = cll_block(pos);
	if (!(_25.result))
	{
		_27 = cll_spaced_block(pos);
		if (!(_27.result))
		{
			return {};
		}
		else 
		{
			success_28 = true;
			_23 = _27.token;
		}
	}
	else 
	{
		success_26 = true;
		_23 = _25.token;
	}
	_23 += ::Parser::str_t(pos, 0);
	success_24 = true;
	pos += 0;
	loop_for_data data;
	data.block = _21;
	data.end = _17;
	data.cond = _13;
	data.decl = _6;

	return {true, ::Parser::Rule(in->startpos, in->start, pos->end, Rules::loop_for, data)};
}
Parser::Rule_res Parser::Parser::method_call(Token*& pos) {
	auto in = pos;
	::Parser::Token_res _0;
	::Parser::bool_t success_1 = false;
	::Parser::arr_t<::Parser::str_t> shadow_11;
	::Parser::str_t _2;
	::Parser::bool_t success_3 = false;
	::Parser::bool_t success_12 = false;
	::Parser::Token_res _4;
	::Parser::bool_t success_5 = false;
	::Parser::arr_t<::Parser::Token_res> shadow_6;
	::Parser::Rule_res _7;
	::Parser::bool_t success_8 = false;
	::Parser::arr_t<::Parser::Rule_res> shadow_9;
	if (!(pos->name == Tokens::ID))
	{
		return {};
	}
	_0 = *pos;
	success_1 = true;
	pos += 1;
	skipspaces(pos);
	auto begin_10 = pos;
	while (1)
	{
		if (!(begin_10->name == Tokens::AUTO_99))
		{
			break;
		}
		_4 = *begin_10;
		success_5 = true;
		begin_10 += 1;
		shadow_6.push_back(_4);
		skipspaces(pos);
		_7 = cll_function_call(begin_10);
		if (!(_7.result))
		{
			break;
		}
		success_8 = true;
		begin_10 += 0;
		shadow_9.push_back(NONE);
		shadow_11.push_back(_2);
		success_12 = true;
	}
	if (!success_12)
	{
		return {};
	}
	if (success_8)
	{
		success_3 = true;
		pos = begin_10;
	}
	_2 += shadow_6;
	_2 += shadow_9;
	method_call_data data;
	data.call = shadow_9;
	data.object = _0;

	return {true, ::Parser::Rule(in->startpos, in->start, pos->end, Rules::method_call, data)};
}
Parser::Rule_res Parser::Parser::copiable_method_call(Token*& pos) {
	auto in = pos;
	::Parser::Token_res _0;
	::Parser::bool_t success_1 = false;
	::Parser::Rule_res _2;
	::Parser::bool_t success_3 = false;
	if (!(pos->name == Tokens::AUTO_55))
	{
		return {};
	}
	_0 = *pos;
	success_1 = true;
	pos += 1;
	skipspaces(pos);
	_2 = method_call(pos);
	if (!(_2.result))
	{
		return {};
	}
	success_3 = true;
	pos += 0;
	copiable_method_call_data data = _2.token;
	return {true, ::Parser::Rule(in->startpos, in->start, pos->end, Rules::copiable_method_call, data)};
}
Parser::Rule_res Parser::Parser::var_refer(Token*& pos) {
	auto in = pos;
	::Parser::any_t pre = matched();
	::Parser::any_t post = matched();
	::Parser::Token _0;
	::Parser::bool_t success_1 = false;
	::Parser::Token _2;
	::Parser::bool_t success_3 = false;
	::Parser::Token_res _4;
	::Parser::bool_t success_5 = false;
	::Parser::Token_res _6;
	::Parser::bool_t success_7 = false;
	::Parser::Token_res _9;
	::Parser::bool_t success_10 = false;
	::Parser::str_t _11;
	::Parser::bool_t success_12 = false;
	::Parser::Token_res _13;
	::Parser::bool_t success_14 = false;
	::Parser::Rule_res _15;
	::Parser::bool_t success_16 = false;
	::Parser::Token_res _17;
	::Parser::bool_t success_18 = false;
	::Parser::Token _20;
	::Parser::bool_t success_21 = false;
	::Parser::Token _22;
	::Parser::bool_t success_23 = false;
	::Parser::Token_res _24;
	::Parser::bool_t success_25 = false;
	::Parser::Token_res _26;
	::Parser::bool_t success_27 = false;
	auto begin_8 = pos;
	do
	{
		if (!(begin_8->name == Tokens::AUTO_60))
		{
			if (!(begin_8->name == Tokens::AUTO_61))
			{
				break;
			}
			else 
			{
				_6 = *begin_8;
				success_7 = true;
				_2 = _6;
			}
		}
		else 
		{
			_4 = *begin_8;
			success_5 = true;
			_2 = _4;
		}
		_2 += ::Parser::str_t(begin_8, 2);
		success_3 = true;
		begin_8 += 2;
	}	while(0)
;
	if (success_3)
	{
		success_1 = true;
		pos = begin_8;
	}
	_0 = _2;
	skipspaces(pos);
	if (!(pos->name == Tokens::ID))
	{
		return {};
	}
	_9 = *pos;
	success_10 = true;
	pos += 1;
	skipspaces(pos);
	auto begin_19 = pos;
	do
	{
		if (!(begin_19->name == Tokens::AUTO_90))
		{
			break;
		}
		_13 = *begin_19;
		success_14 = true;
		begin_19 += 1;
		skipspaces(pos);
		_15 = expr(begin_19);
		if (!(_15.result))
		{
			break;
		}
		success_16 = true;
		begin_19 += 0;
		skipspaces(pos);
		if (!(begin_19->name == Tokens::AUTO_92))
		{
			break;
		}
		_17 = *begin_19;
		success_18 = true;
		begin_19 += 1;
	}	while(0)
;
	if (success_18)
	{
		success_12 = true;
		pos = begin_19;
	}
	_11 += _13;
	_11 += _15.token;
	_11 += _17;
	skipspaces(pos);
	auto begin_28 = pos;
	do
	{
		if (!(begin_28->name == Tokens::AUTO_60))
		{
			if (!(begin_28->name == Tokens::AUTO_61))
			{
				break;
			}
			else 
			{
				_26 = *begin_28;
				success_27 = true;
				_22 = _26;
			}
		}
		else 
		{
			_24 = *begin_28;
			success_25 = true;
			_22 = _24;
		}
		_22 += ::Parser::str_t(begin_28, 2);
		success_23 = true;
		begin_28 += 2;
	}	while(0)
;
	if (success_23)
	{
		success_21 = true;
		pos = begin_28;
	}
	_20 = _22;
	var_refer_data data;
	data.post = post;
	data.pre = pre;
	data.brace_expression = _15.token;
	data.name = _2;

	return {true, ::Parser::Rule(in->startpos, in->start, pos->end, Rules::var_refer, data)};
}
Parser::Rule_res Parser::Parser::cll_var(Token*& pos) {
	auto in = pos;
	::Parser::Rule_res _0;
	::Parser::bool_t success_1 = false;
	::Parser::Token_res _2;
	::Parser::bool_t success_3 = false;
	::Parser::str_t _4;
	::Parser::bool_t success_5 = false;
	::Parser::Rule_res _6;
	::Parser::bool_t success_7 = false;
	::Parser::Rule _8;
	::Parser::bool_t success_9 = false;
	::Parser::Rule_res _10;
	::Parser::bool_t success_11 = false;
	::Parser::Rule_res _12;
	::Parser::bool_t success_13 = false;
	_0 = cll_type(pos);
	if (_0.result)
	{
		success_1 = true;
		pos += 0;
	}
	skipspaces(pos);
	if (!(pos->name == Tokens::ID))
	{
		return {};
	}
	_2 = *pos;
	success_3 = true;
	pos += 1;
	skipspaces(pos);
	auto begin_14 = pos;
	do
	{
		_6 = assignment_op(begin_14);
		if (!(_6.result))
		{
			break;
		}
		success_7 = true;
		begin_14 += 0;
		skipspaces(pos);
		_10 = expr(begin_14);
		if (!(_10.result))
		{
			_12 = cll_ternary(begin_14);
			if (!(_12.result))
			{
				break;
			}
			else 
			{
				success_13 = true;
				_8 = _12.token;
			}
		}
		else 
		{
			success_11 = true;
			_8 = _10.token;
		}
		_8 += ::Parser::str_t(begin_14, 0);
		success_9 = true;
		begin_14 += 0;
	}	while(0)
;
	if (success_9)
	{
		success_5 = true;
		pos = begin_14;
	}
	_4 += _6.token;
	_4 += _8;
	cll_var_data data;
	data.value = _8;
	data.operator = _6.token;
	data.id = _2;
	data.type = _0.token;

	return {true, ::Parser::Rule(in->startpos, in->start, pos->end, Rules::cll_var, data)};
}
Parser::Token_res Parser::Tokenizator::STRING(const char* &pos) {
	auto in = pos;
	data = _4;
	::Parser::str_t _0;
	::Parser::bool_t success_1 = false;
	::Parser::str_t _2;
	::Parser::bool_t success_3 = false;
	auto begin_20 = pos;
	do
	{
		::Parser::str_t _4;
		::Parser::bool_t success_5 = false;
		if (!(*(pos + 0) == '\''))
		{
			break;
		}
		_4 += ::Parser::str_t(begin_20, 1);
		success_5 = true;
		begin_20 += 1;
		skipspaces(pos);
		::Parser::arr_t<::Parser::any_t> shadow_17;
		::Parser::any_t _6;
		::Parser::bool_t success_7 = false;
		auto begin_16 = begin_20;
		while (1)
		{
			::Parser::any_t _8;
			::Parser::bool_t success_9 = false;
			::Parser::str_t _10;
			::Parser::bool_t success_11 = false;
			::Parser::arr_t<::Parser::str_t> shadow_12;
			if (!(!std::strncmp(pos, "\\'", 2)))
			{
				::Parser::str_t _13;
				::Parser::bool_t success_14 = false;
				::Parser::arr_t<::Parser::str_t> shadow_15;
				if (!(!(*(pos + 2) == '\'')))
				{
					break;
				}
				else 
				{
					_13 += ::Parser::str_t(begin_16, 3);
					success_14 = true;
					shadow_15.push_back(_13);
					_8 = shadow_15;
				}
			}
			else 
			{
				_10 += ::Parser::str_t(begin_16, 3);
				success_11 = true;
				shadow_12.push_back(_10);
				_8 = shadow_12;
			}
			_8 += ::Parser::str_t(begin_16, 3);
			success_9 = true;
			begin_16 += 3;
			shadow_17.push_back(_6);
		}
		if (success_9)
		{
			success_7 = true;
			begin_20 = begin_16;
		}
		skipspaces(pos);
		::Parser::str_t _18;
		::Parser::bool_t success_19 = false;
		if (!(*(pos + 0) == '\''))
		{
			break;
		}
		_18 += ::Parser::str_t(begin_20, 1);
		success_19 = true;
		begin_20 += 1;
	}	while(0)
;
	if (success_19)
	{
		success_3 = true;
		pos = begin_20;
	}
	_2 += _4;
	_2 += shadow_17;
	_2 += _18;
	if (!success_3)
	{
		::Parser::str_t _21;
		::Parser::bool_t success_22 = false;
		auto begin_39 = pos;
		do
		{
			::Parser::str_t _23;
			::Parser::bool_t success_24 = false;
			if (!(*(pos + 0) == '"'))
			{
				break;
			}
			_23 += ::Parser::str_t(begin_39, 1);
			success_24 = true;
			begin_39 += 1;
			skipspaces(pos);
			::Parser::arr_t<::Parser::any_t> shadow_36;
			::Parser::any_t _25;
			::Parser::bool_t success_26 = false;
			auto begin_35 = begin_39;
			while (1)
			{
				::Parser::any_t _27;
				::Parser::bool_t success_28 = false;
				::Parser::str_t _29;
				::Parser::bool_t success_30 = false;
				::Parser::arr_t<::Parser::str_t> shadow_31;
				if (!(!std::strncmp(pos, "\\\"", 2)))
				{
					::Parser::str_t _32;
					::Parser::bool_t success_33 = false;
					::Parser::arr_t<::Parser::str_t> shadow_34;
					if (!(!(*(pos + 2) == '"')))
					{
						break;
					}
					else 
					{
						_32 += ::Parser::str_t(begin_35, 3);
						success_33 = true;
						shadow_34.push_back(_32);
						_27 = shadow_34;
					}
				}
				else 
				{
					_29 += ::Parser::str_t(begin_35, 3);
					success_30 = true;
					shadow_31.push_back(_29);
					_27 = shadow_31;
				}
				_27 += ::Parser::str_t(begin_35, 3);
				success_28 = true;
				begin_35 += 3;
				shadow_36.push_back(_25);
			}
			if (success_28)
			{
				success_26 = true;
				begin_39 = begin_35;
			}
			skipspaces(pos);
			::Parser::str_t _37;
			::Parser::bool_t success_38 = false;
			if (!(*(pos + 0) == '"'))
			{
				break;
			}
			_37 += ::Parser::str_t(begin_39, 1);
			success_38 = true;
			begin_39 += 1;
		}		while(0)
;
		if (success_38)
		{
			success_22 = true;
			pos = begin_39;
		}
		_21 += _23;
		_21 += shadow_36;
		_21 += _37;
		if (!success_22)
		{
			return {};
		}
		else 
		{
			_0 = _21;
		}
	}
	else 
	{
		_0 = _2;
	}
	_0 += ::Parser::str_t(pos, 0);
	success_1 = true;
	pos += 0;
	return {};
}
Parser::Token_res Parser::Tokenizator::NUMBER(const char* &pos) {
	auto in = pos;
	char sign = _0;
	::Parser::str_t main = _6;
	::Parser::num_t main_n = NUMBER(main);
	char point = _0;
	::Parser::str_t dec = _0;
	::Parser::num_t dec_n = NUMBER(dec);
	::Parser::str_t full = main+point;
	::Parser::Rule_res _0;
	::Parser::bool_t success_1 = false;
	skipspaces(pos);
	::Parser::str_t _2;
	::Parser::bool_t success_3 = false;
	if (!(*(pos + 0) == '+' || *(pos + 1) == '-'))
	{
		return {};
	}
	_2 += ::Parser::str_t(pos, 2);
	success_3 = true;
	pos += 2;
	skipspaces(pos);
	::Parser::str_t _4;
	::Parser::bool_t success_5 = false;
	::Parser::str_t _6;
	::Parser::bool_t success_7 = false;
	auto begin_12 = pos;
	do
	{
		::Parser::str_t _8;
		::Parser::bool_t success_9 = false;
		::Parser::arr_t<::Parser::str_t> shadow_10;
		::Parser::bool_t success_11 = false;
		while ((*(pos + 0)>='0' && *(pos + 0)<='9'))
		{
			_8 += ::Parser::str_t(begin_12, 1);
			success_9 = true;
			begin_12 += 1;
			shadow_10.push_back(_8);
			success_11 = true;
		}
		if (!success_11)
		{
			break;
		}
	}	while(0)
;
	if (success_9)
	{
		success_7 = true;
		pos = begin_12;
	}
	_6 += shadow_10;
	if (!success_7)
	{
		::Parser::str_t _13;
		::Parser::bool_t success_14 = false;
		auto begin_21 = pos;
		do
		{
			::Parser::str_t _15;
			::Parser::bool_t success_16 = false;
			if (!(*(pos + 0) == '.' || *(pos + 1) == ','))
			{
				break;
			}
			_15 += ::Parser::str_t(begin_21, 2);
			success_16 = true;
			begin_21 += 2;
			skipspaces(pos);
			::Parser::str_t _17;
			::Parser::bool_t success_18 = false;
			::Parser::arr_t<::Parser::str_t> shadow_19;
			::Parser::bool_t success_20 = false;
			while ((*(pos + 0)>='0' && *(pos + 0)<='9'))
			{
				_17 += ::Parser::str_t(begin_21, 1);
				success_18 = true;
				begin_21 += 1;
				shadow_19.push_back(_17);
				success_20 = true;
			}
			if (!success_20)
			{
				break;
			}
		}		while(0)
;
		if (success_18)
		{
			success_14 = true;
			pos = begin_21;
		}
		_13 += _15;
		_13 += shadow_19;
		if (!success_14)
		{
			return {};
		}
		else 
		{
			_4 = _13;
		}
	}
	else 
	{
		_4 = _6;
	}
	_4 += ::Parser::str_t(pos, 0);
	success_5 = true;
	pos += 0;
	NUMBER_data data;
	data.full = full;
	data.dec_n = dec_n;
	data.dec = dec;
	data.main_n = main_n;
	data.main = main;
	data.sign = sign;

	return {true, ::Parser::Token(in - str, in, pos, Tokens::NUMBER, data)};
}
Parser::Token_res Parser::Tokenizator::BOOLEAN(const char* &pos) {
	auto in = pos;
	::Parser::str_t d;
	::Parser::bool_t success_0 = false;
	auto begin_7 = pos;
	::Parser::str_t _1;
	::Parser::bool_t success_2 = false;
	::Parser::str_t _3;
	::Parser::bool_t success_4 = false;
	if (!(!std::strncmp(pos, "true", 4)))
	{
		::Parser::str_t _5;
		::Parser::bool_t success_6 = false;
		if (!(!std::strncmp(pos, "false", 5)))
		{
			return {};
		}
		else 
		{
			_5 += ::Parser::str_t(begin_7, 9);
			success_6 = true;
			_1 = _5;
		}
	}
	else 
	{
		_3 += ::Parser::str_t(begin_7, 9);
		success_4 = true;
		_1 = _3;
	}
	_1 += ::Parser::str_t(begin_7, 9);
	success_2 = true;
	begin_7 += 9;
	if (success_2)
	{
		success_0 = true;
		pos = begin_7;
	}
	d += _1;
	skipspaces(pos);
	::Parser::bool_t val;
	BOOLEAN_data data;
	data.lit = d;
	data.val = val;

	return {true, ::Parser::Token(in - str, in, pos, Tokens::BOOLEAN, data)};
}
Parser::Rule_res Parser::Parser::array(Token*& pos) {
	auto in = pos;
	::Parser::Token_res _0;
	::Parser::bool_t success_1 = false;
	::Parser::str_t _2;
	::Parser::bool_t success_3 = false;
	::Parser::arr_t<::Parser::str_t> shadow_13;
	::Parser::str_t _4;
	::Parser::bool_t success_5 = false;
	::Parser::Rule_res _6;
	::Parser::bool_t success_7 = false;
	::Parser::arr_t<::Parser::Rule_res> shadow_8;
	::Parser::Token_res _9;
	::Parser::bool_t success_10 = false;
	::Parser::arr_t<::Parser::Token_res> shadow_11;
	::Parser::Token_res _15;
	::Parser::bool_t success_16 = false;
	if (!(pos->name == Tokens::AUTO_90))
	{
		return {};
	}
	_0 = *pos;
	success_1 = true;
	pos += 1;
	skipspaces(pos);
	auto begin_14 = pos;
	do
	{
		auto begin_12 = begin_14;
		while (1)
		{
			_6 = any_data(begin_12);
			if (!(_6.result))
			{
				break;
			}
			success_7 = true;
			begin_12 += 0;
			shadow_8.push_back(NONE);
			skipspaces(pos);
			if (!(begin_12->name == Tokens::AUTO_78))
			{
				break;
			}
			_9 = *begin_12;
			success_10 = true;
			begin_12 += 1;
			shadow_11.push_back(_9);
			shadow_13.push_back(_4);
		}
		if (success_10)
		{
			success_5 = true;
			begin_14 = begin_12;
		}
		_4 += shadow_8;
		_4 += shadow_11;
	}	while(0)
;
	if (success_5)
	{
		success_3 = true;
		pos = begin_14;
	}
	_2 += shadow_13;
	skipspaces(pos);
	if (!(pos->name == Tokens::AUTO_92))
	{
		return {};
	}
	_15 = *pos;
	success_16 = true;
	pos += 1;
	array_data data = shadow_11;
	return {true, ::Parser::Rule(in->startpos, in->start, pos->end, Rules::array, data)};
}
Parser::Rule_res Parser::Parser::object(Token*& pos) {
	auto in = pos;
	::Parser::Token_res _0;
	::Parser::bool_t success_1 = false;
	::Parser::str_t _2;
	::Parser::bool_t success_3 = false;
	::Parser::Rule_res _4;
	::Parser::bool_t success_5 = false;
	::Parser::Token_res _6;
	::Parser::bool_t success_7 = false;
	::Parser::Rule_res _8;
	::Parser::bool_t success_9 = false;
	::Parser::arr_t<::Parser::str_t> shadow_25;
	::Parser::str_t _10;
	::Parser::bool_t success_11 = false;
	::Parser::Token_res _12;
	::Parser::bool_t success_13 = false;
	::Parser::arr_t<::Parser::Token_res> shadow_14;
	::Parser::Token_res _15;
	::Parser::bool_t success_16 = false;
	::Parser::arr_t<::Parser::Token_res> shadow_17;
	::Parser::Token_res _18;
	::Parser::bool_t success_19 = false;
	::Parser::arr_t<::Parser::Token_res> shadow_20;
	::Parser::Rule_res _21;
	::Parser::bool_t success_22 = false;
	::Parser::arr_t<::Parser::Rule_res> shadow_23;
	::Parser::Token_res _26;
	::Parser::bool_t success_27 = false;
	::Parser::Token_res _29;
	::Parser::bool_t success_30 = false;
	if (!(pos->name == Tokens::AUTO_65))
	{
		return {};
	}
	_0 = *pos;
	success_1 = true;
	pos += 1;
	skipspaces(pos);
	auto begin_28 = pos;
	do
	{
		_4 = any_data(begin_28);
		if (!(_4.result))
		{
			break;
		}
		success_5 = true;
		begin_28 += 0;
		skipspaces(pos);
		if (!(begin_28->name == Tokens::AUTO_105))
		{
			break;
		}
		_6 = *begin_28;
		success_7 = true;
		begin_28 += 1;
		skipspaces(pos);
		_8 = any_data(begin_28);
		if (!(_8.result))
		{
			break;
		}
		success_9 = true;
		begin_28 += 0;
		skipspaces(pos);
		auto begin_24 = begin_28;
		while (1)
		{
			if (!(begin_24->name == Tokens::AUTO_78))
			{
				break;
			}
			_12 = *begin_24;
			success_13 = true;
			begin_24 += 1;
			shadow_14.push_back(_12);
			skipspaces(pos);
			if (!(begin_24->name == Tokens::ID))
			{
				break;
			}
			_15 = *begin_24;
			success_16 = true;
			begin_24 += 1;
			shadow_17.push_back(_15);
			skipspaces(pos);
			if (!(begin_24->name == Tokens::AUTO_105))
			{
				break;
			}
			_18 = *begin_24;
			success_19 = true;
			begin_24 += 1;
			shadow_20.push_back(_18);
			skipspaces(pos);
			_21 = any_data(begin_24);
			if (!(_21.result))
			{
				break;
			}
			success_22 = true;
			begin_24 += 0;
			shadow_23.push_back(NONE);
			shadow_25.push_back(_10);
		}
		if (success_22)
		{
			success_11 = true;
			begin_28 = begin_24;
		}
		_10 += shadow_14;
		_10 += shadow_17;
		_10 += shadow_20;
		_10 += shadow_23;
		skipspaces(pos);
		if (!(begin_28->name == Tokens::AUTO_103))
		{
			break;
		}
		_26 = *begin_28;
		success_27 = true;
		begin_28 += 1;
	}	while(0)
;
	if (success_27)
	{
		success_3 = true;
		pos = begin_28;
	}
	_2 += _4.token;
	_2 += _6;
	_2 += _8.token;
	_2 += shadow_25;
	_2 += _26;
	skipspaces(pos);
	if (!(pos->name == Tokens::AUTO_70))
	{
		return {};
	}
	_29 = *pos;
	success_30 = true;
	pos += 1;
	object_data data;
	data.values = shadow_23;
	data.keys = shadow_17;
	data.value = _8.token;
	data.key = _4.token;

	return {true, ::Parser::Rule(in->startpos, in->start, pos->end, Rules::object, data)};
}
Parser::Rule_res Parser::Parser::any_data(Token*& pos) {
	auto in = pos;
	::Parser::any_t _0;
	::Parser::bool_t success_1 = false;
	::Parser::Token_res _2;
	::Parser::bool_t success_3 = false;
	::Parser::Token_res _4;
	::Parser::bool_t success_5 = false;
	::Parser::Token_res _6;
	::Parser::bool_t success_7 = false;
	::Parser::Token_res _8;
	::Parser::bool_t success_9 = false;
	::Parser::Rule_res _10;
	::Parser::bool_t success_11 = false;
	::Parser::Rule_res _12;
	::Parser::bool_t success_13 = false;
	::Parser::Rule_res _14;
	::Parser::bool_t success_15 = false;
	data = _0;
	if (!(pos->name == Tokens::BOOLEAN))
	{
		if (!(pos->name == Tokens::NUMBER))
		{
			if (!(pos->name == Tokens::STRING))
			{
				if (!(pos->name == Tokens::ID))
				{
					_10 = array(pos);
					if (!(_10.result))
					{
						_12 = object(pos);
						if (!(_12.result))
						{
							_14 = accessor(pos);
							if (!(_14.result))
							{
								return {};
							}
							else 
							{
								success_15 = true;
								_0 = _14.token;
							}
						}
						else 
						{
							success_13 = true;
							_0 = _12.token;
						}
					}
					else 
					{
						success_11 = true;
						_0 = _10.token;
					}
				}
				else 
				{
					_8 = *pos;
					success_9 = true;
					_0 = _8;
				}
			}
			else 
			{
				_6 = *pos;
				success_7 = true;
				_0 = _6;
			}
		}
		else 
		{
			_4 = *pos;
			success_5 = true;
			_0 = _4;
		}
	}
	else 
	{
		_2 = *pos;
		success_3 = true;
		_0 = _2;
	}
	_0 += ::Parser::str_t(pos, 4);
	success_1 = true;
	pos += 4;
	return {};
}
Parser::Token_res Parser::Tokenizator::END(const char* &pos) {
	auto in = pos;
	::Parser::str_t _0;
	::Parser::bool_t success_1 = false;
	if (!(*(pos + 0) == ';' || *(pos + 1) == '\n'))
	{
		return {};
	}
	_0 += ::Parser::str_t(pos, 2);
	success_1 = true;
	pos += 2;
	return {};
}
Parser::Token_res Parser::Tokenizator::NEWLINE(const char* &pos) {
	auto in = pos;
	::Parser::str_t _0;
	::Parser::bool_t success_1 = false;
	if (!(*(pos + 0) == '\n'))
	{
		return {};
	}
	_0 += ::Parser::str_t(pos, 1);
	success_1 = true;
	pos += 1;
	return {};
}
Parser::Token_res Parser::Tokenizator::ID(const char* &pos) {
	auto in = pos;
	::Parser::str_t _0;
	::Parser::bool_t success_1 = false;
	auto begin_10 = pos;
	::Parser::str_t _2;
	::Parser::bool_t success_3 = false;
	::Parser::arr_t<::Parser::str_t> shadow_4;
	while ((*(pos + 0)>='0' && *(pos + 0)<='9'))
	{
		_2 += ::Parser::str_t(begin_10, 1);
		success_3 = true;
		begin_10 += 1;
		shadow_4.push_back(_2);
	}
	skipspaces(pos);
	::Parser::str_t _5;
	::Parser::bool_t success_6 = false;
	if (!((*(pos + 0)>='a' && *(pos + 0)<='z') || (*(pos + 1)>='A' && *(pos + 1)<='Z') || *(pos + 2) == '_'))
	{
		return {};
	}
	_5 += ::Parser::str_t(begin_10, 3);
	success_6 = true;
	begin_10 += 3;
	skipspaces(pos);
	::Parser::str_t _7;
	::Parser::bool_t success_8 = false;
	::Parser::arr_t<::Parser::str_t> shadow_9;
	while ((*(pos + 0)>='a' && *(pos + 0)<='z') || (*(pos + 1)>='A' && *(pos + 1)<='Z') || (*(pos + 2)>='0' && *(pos + 2)<='9') || *(pos + 3) == '_')
	{
		_7 += ::Parser::str_t(begin_10, 4);
		success_8 = true;
		begin_10 += 4;
		shadow_9.push_back(_7);
	}
	if (success_8)
	{
		success_1 = true;
		pos = begin_10;
	}
	_0 += shadow_4;
	_0 += _5;
	_0 += shadow_9;
	ID_data data = _0;
	return {true, ::Parser::Token(in - str, in, pos, Tokens::ID, data)};
}
Parser::Rule_res Parser::Parser::spacemode(Token*& pos) {
	auto in = pos;
	::Parser::Token_res _0;
	::Parser::bool_t success_1 = false;
	::Parser::Token _2;
	::Parser::bool_t success_3 = false;
	::Parser::Token_res _4;
	::Parser::bool_t success_5 = false;
	::Parser::Token_res _6;
	::Parser::bool_t success_7 = false;
	::Parser::Token_res _8;
	::Parser::bool_t success_9 = false;
	if (!(pos->name == Tokens::AUTO_71))
	{
		return {};
	}
	_0 = *pos;
	success_1 = true;
	pos += 1;
	skipspaces(pos);
	if (!(pos->name == Tokens::AUTO_72))
	{
		if (!(pos->name == Tokens::AUTO_73))
		{
			if (!(pos->name == Tokens::AUTO_74))
			{
				return {};
			}
			else 
			{
				_8 = *pos;
				success_9 = true;
				_2 = _8;
			}
		}
		else 
		{
			_6 = *pos;
			success_7 = true;
			_2 = _6;
		}
	}
	else 
	{
		_4 = *pos;
		success_5 = true;
		_2 = _4;
	}
	_2 += ::Parser::str_t(pos, 3);
	success_3 = true;
	pos += 3;
	spacemode_data data = _0;
	return {true, ::Parser::Rule(in->startpos, in->start, pos->end, Rules::spacemode, data)};
}
Parser::Rule_res Parser::Parser::linear_comment(Token*& pos) {
	auto in = pos;
	::Parser::Token_res _0;
	::Parser::bool_t success_1 = false;
	::Parser::Token_res _2;
	::Parser::bool_t success_3 = false;
	if (!(pos->name == Tokens::AUTO_75))
	{
		return {};
	}
	_0 = *pos;
	success_1 = true;
	pos += 1;
	skipspaces(pos);
	if (!(pos->name == Tokens::AUTO_76))
	{
		return {};
	}
	_2 = *pos;
	success_3 = true;
	pos += 1;
	return {};
}
Parser::Rule_res Parser::Parser::main(Token*& pos) {
	auto in = pos;
	::Parser::Rule _0;
	::Parser::bool_t success_1 = false;
	::Parser::Rule_res _2;
	::Parser::bool_t success_3 = false;
	::Parser::Rule_res _4;
	::Parser::bool_t success_5 = false;
	::Parser::Rule_res _6;
	::Parser::bool_t success_7 = false;
	::Parser::Rule_res _8;
	::Parser::bool_t success_9 = false;
	_2 = Import(pos);
	if (!(_2.result))
	{
		_4 = use(pos);
		if (!(_4.result))
		{
			_6 = Rule(pos);
			if (!(_6.result))
			{
				_8 = spacemode(pos);
				if (!(_8.result))
				{
					return {};
				}
				else 
				{
					success_9 = true;
					_0 = _8.token;
				}
			}
			else 
			{
				success_7 = true;
				_0 = _6.token;
			}
		}
		else 
		{
			success_5 = true;
			_0 = _4.token;
		}
	}
	else 
	{
		success_3 = true;
		_0 = _2.token;
	}
	_0 += ::Parser::str_t(pos, 0);
	success_1 = true;
	pos += 0;
	return {};
}
Parser::Rule_res Parser::Parser::use_unit(Token*& pos) {
	auto in = pos;
	::Parser::Rule_res _0;
	::Parser::bool_t success_1 = false;
	::Parser::Rule_res _2;
	::Parser::bool_t success_3 = false;
	_0 = id(pos);
	if (!(_0.result))
	{
		return {};
	}
	success_1 = true;
	pos += 0;
	skipspaces(pos);
	_2 = any_data(pos);
	if (_2.result)
	{
		success_3 = true;
		pos += 0;
	}
	use_unit_data data;
	data.value = _2.token;
	data.name = _0.token;

	return {true, ::Parser::Rule(in->startpos, in->start, pos->end, Rules::use_unit, data)};
}
Parser::Rule_res Parser::Parser::use(Token*& pos) {
	auto in = pos;
	::Parser::Token_res _0;
	::Parser::bool_t success_1 = false;
	::Parser::Rule_res _2;
	::Parser::bool_t success_3 = false;
	::Parser::arr_t<::Parser::str_t> shadow_13;
	::Parser::str_t _4;
	::Parser::bool_t success_5 = false;
	::Parser::Token_res _6;
	::Parser::bool_t success_7 = false;
	::Parser::arr_t<::Parser::Token_res> shadow_8;
	::Parser::Rule_res _9;
	::Parser::bool_t success_10 = false;
	::Parser::arr_t<::Parser::Rule_res> shadow_11;
	if (!(pos->name == Tokens::AUTO_77))
	{
		return {};
	}
	_0 = *pos;
	success_1 = true;
	pos += 1;
	skipspaces(pos);
	_2 = use_unit(pos);
	if (!(_2.result))
	{
		return {};
	}
	success_3 = true;
	pos += 0;
	skipspaces(pos);
	auto begin_12 = pos;
	while (1)
	{
		if (!(begin_12->name == Tokens::AUTO_78))
		{
			break;
		}
		_6 = *begin_12;
		success_7 = true;
		begin_12 += 1;
		shadow_8.push_back(_6);
		skipspaces(pos);
		_9 = use_unit(begin_12);
		if (!(_9.result))
		{
			break;
		}
		success_10 = true;
		begin_12 += 0;
		shadow_11.push_back(NONE);
		shadow_13.push_back(_4);
	}
	if (success_10)
	{
		success_5 = true;
		pos = begin_12;
	}
	_4 += shadow_8;
	_4 += shadow_11;
	use_data data;
	data.second = shadow_11;
	data.first = _2.token;

	return {true, ::Parser::Rule(in->startpos, in->start, pos->end, Rules::use, data)};
}
Parser::Rule_res Parser::Parser::Rule_rule(Token*& pos) {
	auto in = pos;
	::Parser::any_t _0;
	::Parser::bool_t success_1 = false;
	::Parser::str_t _2;
	::Parser::bool_t success_3 = false;
	::Parser::any_t _4;
	::Parser::bool_t success_5 = false;
	::Parser::any_t _6;
	::Parser::bool_t success_7 = false;
	::Parser::Rule_res _8;
	::Parser::bool_t success_9 = false;
	::Parser::Rule_res _10;
	::Parser::bool_t success_11 = false;
	::Parser::Token_res _12;
	::Parser::bool_t success_13 = false;
	::Parser::Rule_res _14;
	::Parser::bool_t success_15 = false;
	::Parser::Rule_res _16;
	::Parser::bool_t success_17 = false;
	::Parser::Rule_res _18;
	::Parser::bool_t success_19 = false;
	::Parser::Rule_res _20;
	::Parser::bool_t success_21 = false;
	::Parser::Rule_res _23;
	::Parser::bool_t success_24 = false;
	::Parser::Rule_res _26;
	::Parser::bool_t success_27 = false;
	::Parser::Rule_res _28;
	::Parser::bool_t success_29 = false;
	::Parser::Rule_res _30;
	::Parser::bool_t success_31 = false;
	::Parser::Rule_res _32;
	::Parser::bool_t success_33 = false;
	::Parser::Rule_res _34;
	::Parser::bool_t success_35 = false;
	auto begin_25 = pos;
	do
	{
		auto begin_22 = begin_25;
		_8 = Rule_group(begin_22);
		if (!(_8.result))
		{
			_10 = Rule_csequence(begin_22);
			if (!(_10.result))
			{
				if (!(begin_22->name == Tokens::STRING))
				{
					_14 = accessor(begin_22);
					if (!(_14.result))
					{
						_16 = Rule_hex(begin_22);
						if (!(_16.result))
						{
							_18 = Rule_bin(begin_22);
							if (!(_18.result))
							{
								_20 = Rule_other(begin_22);
								if (!(_20.result))
								{
									break;
								}
								else 
								{
									success_21 = true;
									_6 = _20.token;
								}
							}
							else 
							{
								success_19 = true;
								_6 = _18.token;
							}
						}
						else 
						{
							success_17 = true;
							_6 = _16.token;
						}
					}
					else 
					{
						success_15 = true;
						_6 = _14.token;
					}
				}
				else 
				{
					_12 = *begin_22;
					success_13 = true;
					_6 = _12;
				}
			}
			else 
			{
				success_11 = true;
				_6 = _10.token;
			}
		}
		else 
		{
			success_9 = true;
			_6 = _8.token;
		}
		_6 += ::Parser::str_t(begin_22, 1);
		success_7 = true;
		begin_22 += 1;
		if (success_7)
		{
			success_5 = true;
			begin_25 = begin_22;
		}
		skipspaces(pos);
		_23 = Rule_qualifier(begin_25);
		if (_23.result)
		{
			success_24 = true;
			begin_25 += 0;
		}
	}	while(0)
;
	if (success_24)
	{
		success_3 = true;
		pos = begin_25;
	}
	_2 += _4;
	_2 += _23.token;
	if (!success_3)
	{
		_26 = Rule_rule_escaped(pos);
		if (!(_26.result))
		{
			_28 = Rule_rule_any(pos);
			if (!(_28.result))
			{
				_30 = Rule_rule_op(pos);
				if (!(_30.result))
				{
					_32 = cll(pos);
					if (!(_32.result))
					{
						_34 = linear_comment(pos);
						if (!(_34.result))
						{
							return {};
						}
						else 
						{
							success_35 = true;
							_0 = _34.token;
						}
					}
					else 
					{
						success_33 = true;
						_0 = _32.token;
					}
				}
				else 
				{
					success_31 = true;
					_0 = _30.token;
				}
			}
			else 
			{
				success_29 = true;
				_0 = _28.token;
			}
		}
		else 
		{
			success_27 = true;
			_0 = _26.token;
		}
	}
	else 
	{
		_0 = _2;
	}
	_0 += ::Parser::str_t(pos, 0);
	success_1 = true;
	pos += 0;
	Rule_rule_data data;
	data.qualifier = _4;
	data.val = _6;

	return {true, ::Parser::Rule(in->startpos, in->start, pos->end, Rules::Rule_rule, data)};
}
Parser::Rule_res Parser::Parser::Rule_other(Token*& pos) {
	auto in = pos;
	::Parser::bool_t m = matched();
	::Parser::Token_res _0;
	::Parser::bool_t success_1 = false;
	::Parser::Token_res _2;
	::Parser::bool_t success_3 = false;
	::Parser::arr_t<::Parser::str_t> shadow_13;
	::Parser::str_t _4;
	::Parser::bool_t success_5 = false;
	::Parser::Token_res _6;
	::Parser::bool_t success_7 = false;
	::Parser::arr_t<::Parser::Token_res> shadow_8;
	::Parser::Token_res _9;
	::Parser::bool_t success_10 = false;
	::Parser::arr_t<::Parser::Token_res> shadow_11;
	if (pos->name == Tokens::AUTO_80)
	{
		_0 = *pos;
		success_1 = true;
		pos += 1;
	}
	skipspaces(pos);
	if (!(pos->name == Tokens::ID))
	{
		return {};
	}
	_2 = *pos;
	success_3 = true;
	pos += 1;
	skipspaces(pos);
	auto begin_12 = pos;
	while (1)
	{
		if (!(begin_12->name == Tokens::AUTO_81))
		{
			break;
		}
		_6 = *begin_12;
		success_7 = true;
		begin_12 += 1;
		shadow_8.push_back(_6);
		skipspaces(pos);
		if (!(begin_12->name == Tokens::ID))
		{
			break;
		}
		_9 = *begin_12;
		success_10 = true;
		begin_12 += 1;
		shadow_11.push_back(_9);
		shadow_13.push_back(_4);
	}
	if (success_10)
	{
		success_5 = true;
		pos = begin_12;
	}
	_4 += shadow_8;
	_4 += shadow_11;
	Rule_other_data data;
	data.nested_name = shadow_11;
	data.name = _2;
	data.is_nested = m;

	return {true, ::Parser::Rule(in->startpos, in->start, pos->end, Rules::Rule_other, data)};
}
Parser::Rule_res Parser::Parser::Rule_op(Token*& pos) {
	auto in = pos;
	::Parser::Token _0;
	::Parser::bool_t success_1 = false;
	::Parser::Token _2;
	::Parser::bool_t success_3 = false;
	::Parser::Token_res _4;
	::Parser::bool_t success_5 = false;
	::Parser::Token_res _6;
	::Parser::bool_t success_7 = false;
	data = _0;
	auto begin_8 = pos;
	if (!(begin_8->name == Tokens::AUTO_82))
	{
		if (!(begin_8->name == Tokens::AUTO_83))
		{
			return {};
		}
		else 
		{
			_6 = *begin_8;
			success_7 = true;
			_2 = _6;
		}
	}
	else 
	{
		_4 = *begin_8;
		success_5 = true;
		_2 = _4;
	}
	_2 += ::Parser::str_t(begin_8, 2);
	success_3 = true;
	begin_8 += 2;
	if (success_3)
	{
		success_1 = true;
		pos = begin_8;
	}
	_0 = _2;
	return {};
}
Parser::Rule_res Parser::Parser::Rule_qualifier(Token*& pos) {
	auto in = pos;
	::Parser::Token _0;
	::Parser::bool_t success_1 = false;
	::Parser::Token _2;
	::Parser::bool_t success_3 = false;
	::Parser::Token_res _4;
	::Parser::bool_t success_5 = false;
	::Parser::Token_res _6;
	::Parser::bool_t success_7 = false;
	::Parser::Token_res _8;
	::Parser::bool_t success_9 = false;
	data = _0;
	auto begin_10 = pos;
	if (!(begin_10->name == Tokens::AUTO_84))
	{
		if (!(begin_10->name == Tokens::AUTO_85))
		{
			if (!(begin_10->name == Tokens::AUTO_86))
			{
				return {};
			}
			else 
			{
				_8 = *begin_10;
				success_9 = true;
				_2 = _8;
			}
		}
		else 
		{
			_6 = *begin_10;
			success_7 = true;
			_2 = _6;
		}
	}
	else 
	{
		_4 = *begin_10;
		success_5 = true;
		_2 = _4;
	}
	_2 += ::Parser::str_t(begin_10, 3);
	success_3 = true;
	begin_10 += 3;
	if (success_3)
	{
		success_1 = true;
		pos = begin_10;
	}
	_0 = _2;
	return {};
}
Parser::Rule_res Parser::Parser::Rule_group(Token*& pos) {
	auto in = pos;
	::Parser::any_t name = _2;
	::Parser::any_t r = _11;
	::Parser::str_t _0;
	::Parser::bool_t success_1 = false;
	::Parser::Token_res _2;
	::Parser::bool_t success_3 = false;
	::Parser::any_t _4;
	::Parser::bool_t success_5 = false;
	::Parser::Rule_res _6;
	::Parser::bool_t success_7 = false;
	::Parser::Token_res _8;
	::Parser::bool_t success_9 = false;
	::Parser::Token_res _11;
	::Parser::bool_t success_12 = false;
	::Parser::Rule_res _13;
	::Parser::bool_t success_14 = false;
	::Parser::Token_res _15;
	::Parser::bool_t success_16 = false;
	auto begin_10 = pos;
	do
	{
		if (!(begin_10->name == Tokens::AUTO_87))
		{
			break;
		}
		_2 = *begin_10;
		success_3 = true;
		begin_10 += 1;
		skipspaces(pos);
		_6 = method_call(begin_10);
		if (!(_6.result))
		{
			if (!(begin_10->name == Tokens::ID))
			{
				break;
			}
			else 
			{
				_8 = *begin_10;
				success_9 = true;
				_4 = _8;
			}
		}
		else 
		{
			success_7 = true;
			_4 = _6.token;
		}
		_4 += ::Parser::str_t(begin_10, 1);
		success_5 = true;
		begin_10 += 1;
	}	while(0)
;
	if (success_5)
	{
		success_1 = true;
		pos = begin_10;
	}
	_0 += _2;
	_0 += _4;
	skipspaces(pos);
	if (!(pos->name == Tokens::AUTO_88))
	{
		return {};
	}
	_11 = *pos;
	success_12 = true;
	pos += 1;
	skipspaces(pos);
	_13 = Rule_rule(pos);
	while (_13.result)
	{
		success_14 = true;
		pos += 0;
		_13 = Rule_rule(pos);
	}
	skipspaces(pos);
	if (!(pos->name == Tokens::AUTO_89))
	{
		return {};
	}
	_15 = *pos;
	success_16 = true;
	pos += 1;
	Rule_group_data data;
	data.val = r;
	data.variable = name;

	return {true, ::Parser::Rule(in->startpos, in->start, pos->end, Rules::Rule_group, data)};
}
Parser::Rule_res Parser::Parser::Rule_csequence_symbol(Token*& pos) {
	auto in = pos;
	::Parser::Token _0;
	::Parser::bool_t success_1 = false;
	::Parser::Token _2;
	::Parser::bool_t success_3 = false;
	::Parser::Token_res _4;
	::Parser::bool_t success_5 = false;
	::Parser::Token_res _6;
	::Parser::bool_t success_7 = false;
	::Parser::Token_res _8;
	::Parser::bool_t success_9 = false;
	data = _0;
	auto begin_10 = pos;
	if (!(begin_10->name == Tokens::AUTO_93))
	{
		if (!(begin_10->name == Tokens::AUTO_94))
		{
			if (!(begin_10->name == Tokens::AUTO_95))
			{
				return {};
			}
			else 
			{
				_8 = *begin_10;
				success_9 = true;
				_2 = _8;
			}
		}
		else 
		{
			_6 = *begin_10;
			success_7 = true;
			_2 = _6;
		}
	}
	else 
	{
		_4 = *begin_10;
		success_5 = true;
		_2 = _4;
	}
	_2 += ::Parser::str_t(begin_10, 3);
	success_3 = true;
	begin_10 += 3;
	if (success_3)
	{
		success_1 = true;
		pos = begin_10;
	}
	_0 = _2;
	return {};
}
Parser::Rule_res Parser::Parser::Rule_csequence_escape(Token*& pos) {
	auto in = pos;
	::Parser::Token_res _0;
	::Parser::bool_t success_1 = false;
	::Parser::Token _2;
	::Parser::bool_t success_3 = false;
	::Parser::Token_res _4;
	::Parser::bool_t success_5 = false;
	data = _2;
	if (!(pos->name == Tokens::AUTO_96))
	{
		return {};
	}
	_0 = *pos;
	success_1 = true;
	pos += 1;
	auto begin_6 = pos;
	if (!(begin_6->name == Tokens::AUTO_97))
	{
		return {};
	}
	_4 = *begin_6;
	success_5 = true;
	begin_6 += 1;
	if (success_5)
	{
		success_3 = true;
		pos = begin_6;
	}
	_2 = _4;
	return {};
}
Parser::Rule_res Parser::Parser::Rule_csequence_diapason(Token*& pos) {
	auto in = pos;
	::Parser::str_t _0;
	::Parser::bool_t success_1 = false;
	::Parser::Rule from;
	::Parser::bool_t success_2 = false;
	::Parser::Rule_res _3;
	::Parser::bool_t success_4 = false;
	::Parser::Token_res _6;
	::Parser::bool_t success_7 = false;
	::Parser::Rule to;
	::Parser::bool_t success_8 = false;
	::Parser::Rule_res _9;
	::Parser::bool_t success_10 = false;
	auto begin_12 = pos;
	auto begin_5 = begin_12;
	_3 = Rule_csequence_symbol(begin_5);
	if (!(_3.result))
	{
		return {};
	}
	success_4 = true;
	begin_5 += 0;
	if (success_4)
	{
		success_2 = true;
		begin_12 = begin_5;
	}
	from = _3.token;
	if (!(begin_12->name == Tokens::AUTO_98))
	{
		return {};
	}
	_6 = *begin_12;
	success_7 = true;
	begin_12 += 1;
	auto begin_11 = begin_12;
	_9 = Rule_csequence_symbol(begin_11);
	if (!(_9.result))
	{
		return {};
	}
	success_10 = true;
	begin_11 += 0;
	if (success_10)
	{
		success_8 = true;
		begin_12 = begin_11;
	}
	to = _9.token;
	if (success_8)
	{
		success_1 = true;
		pos = begin_12;
	}
	_0 += from;
	_0 += _6;
	_0 += to;
	Rule_csequence_diapason_data data = {from,to};
	return {true, ::Parser::Rule(in->startpos, in->start, pos->end, Rules::Rule_csequence_diapason, data)};
}
Parser::Rule_res Parser::Parser::Rule_csequence(Token*& pos) {
	auto in = pos;
	::Parser::bool_t m = matched();
	::Parser::Token_res _0;
	::Parser::bool_t success_1 = false;
	::Parser::Token_res _2;
	::Parser::bool_t success_3 = false;
	::Parser::arr_t<::Parser::Rule> shadow_17;
	::Parser::Rule dt;
	::Parser::bool_t success_4 = false;
	::Parser::Rule _5;
	::Parser::bool_t success_6 = false;
	::Parser::Rule_res _7;
	::Parser::bool_t success_8 = false;
	::Parser::arr_t<::Parser::Rule_res> shadow_9;
	::Parser::Rule_res _10;
	::Parser::bool_t success_11 = false;
	::Parser::arr_t<::Parser::Rule_res> shadow_12;
	::Parser::Rule_res _13;
	::Parser::bool_t success_14 = false;
	::Parser::arr_t<::Parser::Rule_res> shadow_15;
	::Parser::Token_res _18;
	::Parser::bool_t success_19 = false;
	if (!(pos->name == Tokens::AUTO_90))
	{
		return {};
	}
	_0 = *pos;
	success_1 = true;
	pos += 1;
	skipspaces(pos);
	if (pos->name == Tokens::AUTO_91)
	{
		_2 = *pos;
		success_3 = true;
		pos += 1;
	}
	skipspaces(pos);
	auto begin_16 = pos;
	while (1)
	{
		_7 = Rule_csequence_escape(begin_16);
		if (!(_7.result))
		{
			_10 = Rule_csequence_symbol(begin_16);
			if (!(_10.result))
			{
				_13 = Rule_csequence_diapason(begin_16);
				if (!(_13.result))
				{
					break;
				}
				else 
				{
					success_14 = true;
					shadow_15.push_back(NONE);
					_5 = shadow_15;
				}
			}
			else 
			{
				success_11 = true;
				shadow_12.push_back(NONE);
				_5 = shadow_12;
			}
		}
		else 
		{
			success_8 = true;
			shadow_9.push_back(NONE);
			_5 = shadow_9;
		}
		_5 += ::Parser::str_t(begin_16, 0);
		success_6 = true;
		begin_16 += 0;
		shadow_17.push_back(dt);
	}
	if (success_6)
	{
		success_4 = true;
		pos = begin_16;
	}
	dt = _5;
	skipspaces(pos);
	if (!(pos->name == Tokens::AUTO_92))
	{
		return {};
	}
	_18 = *pos;
	success_19 = true;
	pos += 1;
	Rule_csequence_data data;
	data.val = dt;
	data.not = m;

	return {true, ::Parser::Rule(in->startpos, in->start, pos->end, Rules::Rule_csequence, data)};
}
Parser::Rule_res Parser::Parser::Rule_data_block_inclosed_map(Token*& pos) {
	auto in = pos;
	::Parser::Rule data;
	::Parser::bool_t success_0 = false;
	::Parser::Rule_res _1;
	::Parser::bool_t success_2 = false;
	::Parser::bool_t success_3 = false;
	auto begin_4 = pos;
	_1 = Rule_data_block_key(begin_4);
	while (_1.result)
	{
		success_2 = true;
		begin_4 += 0;
		_1 = Rule_data_block_key(begin_4);
		success_3 = true;
	}
	if (!success_3)
	{
		return {};
	}
	if (success_2)
	{
		success_0 = true;
		pos = begin_4;
	}
	data = _1.token;
	return {};
}
Parser::Rule_res Parser::Parser::Rule_data_block_key(Token*& pos) {
	auto in = pos;
	::Parser::Token_res _0;
	::Parser::bool_t success_1 = false;
	::Parser::arr_t<::Parser::Token_res> shadow_2;
	::Parser::bool_t success_3 = false;
	::Parser::Rule name;
	::Parser::bool_t success_4 = false;
	::Parser::Rule_res _5;
	::Parser::bool_t success_6 = false;
	::Parser::Token_res _8;
	::Parser::bool_t success_9 = false;
	::Parser::Rule dt;
	::Parser::bool_t success_10 = false;
	::Parser::Rule _11;
	::Parser::bool_t success_12 = false;
	::Parser::Rule_res _13;
	::Parser::bool_t success_14 = false;
	::Parser::Rule_res _15;
	::Parser::bool_t success_16 = false;
	while (pos->name == Tokens::AUTO_104)
	{
		_0 += *pos;
		success_1 = true;
		pos += 1;
		shadow_2.push_back(_0);
		success_3 = true;
	}
	if (!success_3)
	{
		return {};
	}
	skipspaces(pos);
	auto begin_7 = pos;
	_5 = id(begin_7);
	if (!(_5.result))
	{
		return {};
	}
	success_6 = true;
	begin_7 += 0;
	if (success_6)
	{
		success_4 = true;
		pos = begin_7;
	}
	name = _5.token;
	skipspaces(pos);
	if (!(pos->name == Tokens::AUTO_105))
	{
		return {};
	}
	_8 = *pos;
	success_9 = true;
	pos += 1;
	skipspaces(pos);
	auto begin_17 = pos;
	_13 = cll_ternary(begin_17);
	if (!(_13.result))
	{
		_15 = expr(begin_17);
		if (!(_15.result))
		{
			return {};
		}
		else 
		{
			success_16 = true;
			_11 = _15.token;
		}
	}
	else 
	{
		success_14 = true;
		_11 = _13.token;
	}
	_11 += ::Parser::str_t(begin_17, 0);
	success_12 = true;
	begin_17 += 0;
	if (success_12)
	{
		success_10 = true;
		pos = begin_17;
	}
	dt = _11;
	Rule_data_block_key_data data;
	data.val = dt;
	data.name = name;

	return {true, ::Parser::Rule(in->startpos, in->start, pos->end, Rules::Rule_data_block_key, data)};
}
Parser::Rule_res Parser::Parser::Rule_data_block(Token*& pos) {
	auto in = pos;
	::Parser::Token_res _0;
	::Parser::bool_t success_1 = false;
	::Parser::arr_t<::Parser::Token_res> shadow_2;
	::Parser::bool_t success_3 = false;
	::Parser::Token_res _4;
	::Parser::bool_t success_5 = false;
	::Parser::Token_res _6;
	::Parser::bool_t success_7 = false;
	::Parser::Rule data;
	::Parser::bool_t success_8 = false;
	::Parser::Rule _9;
	::Parser::bool_t success_10 = false;
	::Parser::Rule_res _11;
	::Parser::bool_t success_12 = false;
	::Parser::Rule_res _13;
	::Parser::bool_t success_14 = false;
	::Parser::Token_res _16;
	::Parser::bool_t success_17 = false;
	while (pos->name == Tokens::AUTO_100)
	{
		_0 += *pos;
		success_1 = true;
		pos += 1;
		shadow_2.push_back(_0);
		success_3 = true;
	}
	if (!success_3)
	{
		return {};
	}
	skipspaces(pos);
	if (!(pos->name == Tokens::AUTO_101))
	{
		return {};
	}
	_4 = *pos;
	success_5 = true;
	pos += 1;
	skipspaces(pos);
	if (!(pos->name == Tokens::AUTO_102))
	{
		return {};
	}
	_6 = *pos;
	success_7 = true;
	pos += 1;
	skipspaces(pos);
	auto begin_15 = pos;
	_11 = any_data(begin_15);
	if (!(_11.result))
	{
		_13 = Rule_data_block_inclosed_map(begin_15);
		if (!(_13.result))
		{
			return {};
		}
		else 
		{
			success_14 = true;
			_9 = _13.token;
		}
	}
	else 
	{
		success_12 = true;
		_9 = _11.token;
	}
	_9 += ::Parser::str_t(begin_15, 0);
	success_10 = true;
	begin_15 += 0;
	if (success_10)
	{
		success_8 = true;
		pos = begin_15;
	}
	data = _9;
	skipspaces(pos);
	if (!(pos->name == Tokens::AUTO_103))
	{
		return {};
	}
	_16 = *pos;
	success_17 = true;
	pos += 1;
	return {};
}
Parser::Rule_res Parser::Parser::Rule_escaped(Token*& pos) {
	auto in = pos;
	::Parser::Token_res _0;
	::Parser::bool_t success_1 = false;
	::Parser::Token_res _2;
	::Parser::bool_t success_3 = false;
	::Parser::Token_res _4;
	::Parser::bool_t success_5 = false;
	if (!(pos->name == Tokens::AUTO_106))
	{
		return {};
	}
	_0 = *pos;
	success_1 = true;
	pos += 1;
	if (!(pos->name == Tokens::AUTO_107))
	{
		return {};
	}
	_2 = *pos;
	success_3 = true;
	pos += 1;
	if (pos->name == Tokens::NUMBER)
	{
		_4 = *pos;
		success_5 = true;
		pos += 1;
	}
	Rule_escaped_data data;
	data.num = _4;
	data.c = _2;

	return {true, ::Parser::Rule(in->startpos, in->start, pos->end, Rules::Rule_escaped, data)};
}
Parser::Rule_res Parser::Parser::Rule_nested_rule(Token*& pos) {
	auto in = pos;
	::Parser::Token_res _0;
	::Parser::bool_t success_1 = false;
	::Parser::arr_t<::Parser::Token_res> shadow_2;
	::Parser::bool_t success_3 = false;
	::Parser::Token_res _4;
	::Parser::bool_t success_5 = false;
	::Parser::Rule_res _6;
	::Parser::bool_t success_7 = false;
	while (pos->name == Tokens::AUTO_108)
	{
		_0 += *pos;
		success_1 = true;
		pos += 1;
		shadow_2.push_back(_0);
		success_3 = true;
	}
	if (!success_3)
	{
		return {};
	}
	skipspaces(pos);
	if (!(pos->name == Tokens::AUTO_109))
	{
		return {};
	}
	_4 = *pos;
	success_5 = true;
	pos += 1;
	_6 = Rule(pos);
	if (!(_6.result))
	{
		return {};
	}
	success_7 = true;
	pos += 0;
	Rule_nested_rule_data data = _6.token;
	return {true, ::Parser::Rule(in->startpos, in->start, pos->end, Rules::Rule_nested_rule, data)};
}
Parser::Rule_res Parser::Parser::Rule_hex(Token*& pos) {
	auto in = pos;
	::Parser::Token_res _0;
	::Parser::bool_t success_1 = false;
	::Parser::Token_res _2;
	::Parser::bool_t success_3 = false;
	::Parser::arr_t<::Parser::Token_res> shadow_4;
	::Parser::bool_t success_5 = false;
	if (!(pos->name == Tokens::AUTO_110))
	{
		return {};
	}
	_0 = *pos;
	success_1 = true;
	pos += 1;
	skipspaces(pos);
	while (pos->name == Tokens::AUTO_111)
	{
		_2 += *pos;
		success_3 = true;
		pos += 1;
		shadow_4.push_back(_2);
		success_5 = true;
	}
	if (!success_5)
	{
		return {};
	}
	Rule_hex_data data = shadow_4;
	return {true, ::Parser::Rule(in->startpos, in->start, pos->end, Rules::Rule_hex, data)};
}
Parser::Rule_res Parser::Parser::Rule_bin(Token*& pos) {
	auto in = pos;
	::Parser::Token_res _0;
	::Parser::bool_t success_1 = false;
	::Parser::Token_res _2;
	::Parser::bool_t success_3 = false;
	::Parser::arr_t<::Parser::Token_res> shadow_4;
	::Parser::bool_t success_5 = false;
	if (!(pos->name == Tokens::AUTO_112))
	{
		return {};
	}
	_0 = *pos;
	success_1 = true;
	pos += 1;
	skipspaces(pos);
	while (pos->name == Tokens::AUTO_113)
	{
		_2 += *pos;
		success_3 = true;
		pos += 1;
		shadow_4.push_back(_2);
		success_5 = true;
	}
	if (!success_5)
	{
		return {};
	}
	Rule_bin_data data = shadow_4;
	return {true, ::Parser::Rule(in->startpos, in->start, pos->end, Rules::Rule_bin, data)};
}
Parser::Rule_res Parser::Parser::Rule(Token*& pos) {
	auto in = pos;
	::Parser::Token_res _0;
	::Parser::bool_t success_1 = false;
	::Parser::Token_res _2;
	::Parser::bool_t success_3 = false;
	::Parser::arr_t<::Parser::Rule> shadow_10;
	::Parser::Rule _4;
	::Parser::bool_t success_5 = false;
	::Parser::Rule_res _6;
	::Parser::bool_t success_7 = false;
	::Parser::arr_t<::Parser::Rule_res> shadow_8;
	::Parser::Rule_res _11;
	::Parser::bool_t success_12 = false;
	::Parser::Rule_res _13;
	::Parser::bool_t success_14 = false;
	::Parser::Token_res _15;
	::Parser::bool_t success_16 = false;
	if (!(pos->name == Tokens::ID))
	{
		return {};
	}
	_0 = *pos;
	success_1 = true;
	pos += 1;
	skipspaces(pos);
	if (!(pos->name == Tokens::AUTO_105))
	{
		return {};
	}
	_2 = *pos;
	success_3 = true;
	pos += 1;
	skipspaces(pos);
	auto begin_9 = pos;
	while (1)
	{
		_6 = Rule_rule(begin_9);
		if (!(_6.result))
		{
			break;
		}
		success_7 = true;
		begin_9 += 0;
		shadow_8.push_back(NONE);
		shadow_10.push_back(_4);
	}
	if (success_7)
	{
		success_5 = true;
		pos = begin_9;
	}
	_4 = _6.token;
	skipspaces(pos);
	_11 = Rule_data_block(pos);
	if (!(_11.result))
	{
		return {};
	}
	success_12 = true;
	pos += 0;
	skipspaces(pos);
	_13 = Rule_nested_rule(pos);
	while (_13.result)
	{
		success_14 = true;
		pos += 0;
		_13 = Rule_nested_rule(pos);
	}
	skipspaces(pos);
	if (!(pos->name == Tokens::AUTO_103))
	{
		return {};
	}
	_15 = *pos;
	success_16 = true;
	pos += 1;
	Rule_data data;
	data.nestedRules = _11.token;
	data.data_block = shadow_10;
	data.rule = shadow_8;
	data.name = _0;

	return {true, ::Parser::Rule(in->startpos, in->start, pos->end, Rules::Rule, data)};
}
Parser::Token_res Parser::Tokenizator::AUTO_1(const char* &pos) {
	auto in = pos;
	::Parser::str_t _0;
	::Parser::bool_t success_1 = false;
	if (!(*(pos + 0) == '%'))
	{
		return {};
	}
	_0 += ::Parser::str_t(pos, 1);
	success_1 = true;
	pos += 1;
	AUTO_1_data data = _0;
	return {true, ::Parser::Token(in - str, in, pos, Tokens::AUTO_1, data)};
}
Parser::Token_res Parser::Tokenizator::AUTO_3(const char* &pos) {
	auto in = pos;
	::Parser::str_t _0;
	::Parser::bool_t success_1 = false;
	if (!(*(pos + 0) == '>'))
	{
		return {};
	}
	_0 += ::Parser::str_t(pos, 1);
	success_1 = true;
	pos += 1;
	AUTO_3_data data = _0;
	return {true, ::Parser::Token(in - str, in, pos, Tokens::AUTO_3, data)};
}
Parser::Token_res Parser::Tokenizator::AUTO_4(const char* &pos) {
	auto in = pos;
	::Parser::str_t _0;
	::Parser::bool_t success_1 = false;
	if (!(*(pos + 0) == '$'))
	{
		return {};
	}
	_0 += ::Parser::str_t(pos, 1);
	success_1 = true;
	pos += 1;
	AUTO_4_data data = _0;
	return {true, ::Parser::Token(in - str, in, pos, Tokens::AUTO_4, data)};
}
Parser::Token_res Parser::Tokenizator::AUTO_14(const char* &pos) {
	auto in = pos;
	::Parser::str_t _0;
	::Parser::bool_t success_1 = false;
	if (!(!std::strncmp(pos, "unsigned", 8)))
	{
		return {};
	}
	_0 += ::Parser::str_t(pos, 8);
	success_1 = true;
	pos += 8;
	AUTO_14_data data = _0;
	return {true, ::Parser::Token(in - str, in, pos, Tokens::AUTO_14, data)};
}
Parser::Token_res Parser::Tokenizator::AUTO_15(const char* &pos) {
	auto in = pos;
	::Parser::str_t _0;
	::Parser::bool_t success_1 = false;
	if (!(!std::strncmp(pos, "char", 4)))
	{
		return {};
	}
	_0 += ::Parser::str_t(pos, 4);
	success_1 = true;
	pos += 4;
	AUTO_15_data data = _0;
	return {true, ::Parser::Token(in - str, in, pos, Tokens::AUTO_15, data)};
}
Parser::Token_res Parser::Tokenizator::AUTO_16(const char* &pos) {
	auto in = pos;
	::Parser::str_t _0;
	::Parser::bool_t success_1 = false;
	if (!(!std::strncmp(pos, "short", 5)))
	{
		return {};
	}
	_0 += ::Parser::str_t(pos, 5);
	success_1 = true;
	pos += 5;
	AUTO_16_data data = _0;
	return {true, ::Parser::Token(in - str, in, pos, Tokens::AUTO_16, data)};
}
Parser::Token_res Parser::Tokenizator::AUTO_17(const char* &pos) {
	auto in = pos;
	::Parser::str_t _0;
	::Parser::bool_t success_1 = false;
	if (!(!std::strncmp(pos, "int", 3)))
	{
		return {};
	}
	_0 += ::Parser::str_t(pos, 3);
	success_1 = true;
	pos += 3;
	AUTO_17_data data = _0;
	return {true, ::Parser::Token(in - str, in, pos, Tokens::AUTO_17, data)};
}
Parser::Token_res Parser::Tokenizator::AUTO_18(const char* &pos) {
	auto in = pos;
	::Parser::str_t _0;
	::Parser::bool_t success_1 = false;
	if (!(!std::strncmp(pos, "long", 4)))
	{
		return {};
	}
	_0 += ::Parser::str_t(pos, 4);
	success_1 = true;
	pos += 4;
	AUTO_18_data data = _0;
	return {true, ::Parser::Token(in - str, in, pos, Tokens::AUTO_18, data)};
}
Parser::Token_res Parser::Tokenizator::AUTO_19(const char* &pos) {
	auto in = pos;
	::Parser::str_t _0;
	::Parser::bool_t success_1 = false;
	if (!(!std::strncmp(pos, "long long", 9)))
	{
		return {};
	}
	_0 += ::Parser::str_t(pos, 9);
	success_1 = true;
	pos += 9;
	AUTO_19_data data = _0;
	return {true, ::Parser::Token(in - str, in, pos, Tokens::AUTO_19, data)};
}
Parser::Token_res Parser::Tokenizator::AUTO_20(const char* &pos) {
	auto in = pos;
	::Parser::str_t _0;
	::Parser::bool_t success_1 = false;
	if (!(!std::strncmp(pos, "float", 5)))
	{
		return {};
	}
	_0 += ::Parser::str_t(pos, 5);
	success_1 = true;
	pos += 5;
	AUTO_20_data data = _0;
	return {true, ::Parser::Token(in - str, in, pos, Tokens::AUTO_20, data)};
}
Parser::Token_res Parser::Tokenizator::AUTO_21(const char* &pos) {
	auto in = pos;
	::Parser::str_t _0;
	::Parser::bool_t success_1 = false;
	if (!(!std::strncmp(pos, "double", 6)))
	{
		return {};
	}
	_0 += ::Parser::str_t(pos, 6);
	success_1 = true;
	pos += 6;
	AUTO_21_data data = _0;
	return {true, ::Parser::Token(in - str, in, pos, Tokens::AUTO_21, data)};
}
Parser::Token_res Parser::Tokenizator::AUTO_22(const char* &pos) {
	auto in = pos;
	::Parser::str_t _0;
	::Parser::bool_t success_1 = false;
	if (!(!std::strncmp(pos, "deque", 5)))
	{
		return {};
	}
	_0 += ::Parser::str_t(pos, 5);
	success_1 = true;
	pos += 5;
	AUTO_22_data data = _0;
	return {true, ::Parser::Token(in - str, in, pos, Tokens::AUTO_22, data)};
}
Parser::Token_res Parser::Tokenizator::AUTO_23(const char* &pos) {
	auto in = pos;
	::Parser::str_t _0;
	::Parser::bool_t success_1 = false;
	if (!(!std::strncmp(pos, "vector", 6)))
	{
		return {};
	}
	_0 += ::Parser::str_t(pos, 6);
	success_1 = true;
	pos += 6;
	AUTO_23_data data = _0;
	return {true, ::Parser::Token(in - str, in, pos, Tokens::AUTO_23, data)};
}
Parser::Token_res Parser::Tokenizator::AUTO_24(const char* &pos) {
	auto in = pos;
	::Parser::str_t _0;
	::Parser::bool_t success_1 = false;
	if (!(!std::strncmp(pos, "queue", 5)))
	{
		return {};
	}
	_0 += ::Parser::str_t(pos, 5);
	success_1 = true;
	pos += 5;
	AUTO_24_data data = _0;
	return {true, ::Parser::Token(in - str, in, pos, Tokens::AUTO_24, data)};
}
Parser::Token_res Parser::Tokenizator::AUTO_25(const char* &pos) {
	auto in = pos;
	::Parser::str_t _0;
	::Parser::bool_t success_1 = false;
	if (!(!std::strncmp(pos, "stack", 5)))
	{
		return {};
	}
	_0 += ::Parser::str_t(pos, 5);
	success_1 = true;
	pos += 5;
	AUTO_25_data data = _0;
	return {true, ::Parser::Token(in - str, in, pos, Tokens::AUTO_25, data)};
}
Parser::Token_res Parser::Tokenizator::AUTO_26(const char* &pos) {
	auto in = pos;
	::Parser::str_t _0;
	::Parser::bool_t success_1 = false;
	if (!(!std::strncmp(pos, "list", 4)))
	{
		return {};
	}
	_0 += ::Parser::str_t(pos, 4);
	success_1 = true;
	pos += 4;
	AUTO_26_data data = _0;
	return {true, ::Parser::Token(in - str, in, pos, Tokens::AUTO_26, data)};
}
Parser::Token_res Parser::Tokenizator::AUTO_27(const char* &pos) {
	auto in = pos;
	::Parser::str_t _0;
	::Parser::bool_t success_1 = false;
	if (!(!std::strncmp(pos, "forward_list", 12)))
	{
		return {};
	}
	_0 += ::Parser::str_t(pos, 12);
	success_1 = true;
	pos += 12;
	AUTO_27_data data = _0;
	return {true, ::Parser::Token(in - str, in, pos, Tokens::AUTO_27, data)};
}
Parser::Token_res Parser::Tokenizator::AUTO_28(const char* &pos) {
	auto in = pos;
	::Parser::str_t _0;
	::Parser::bool_t success_1 = false;
	if (!(!std::strncmp(pos, "var", 3)))
	{
		return {};
	}
	_0 += ::Parser::str_t(pos, 3);
	success_1 = true;
	pos += 3;
	AUTO_28_data data = _0;
	return {true, ::Parser::Token(in - str, in, pos, Tokens::AUTO_28, data)};
}
Parser::Token_res Parser::Tokenizator::AUTO_29(const char* &pos) {
	auto in = pos;
	::Parser::str_t _0;
	::Parser::bool_t success_1 = false;
	if (!(!std::strncmp(pos, "any", 3)))
	{
		return {};
	}
	_0 += ::Parser::str_t(pos, 3);
	success_1 = true;
	pos += 3;
	AUTO_29_data data = _0;
	return {true, ::Parser::Token(in - str, in, pos, Tokens::AUTO_29, data)};
}
Parser::Token_res Parser::Tokenizator::AUTO_30(const char* &pos) {
	auto in = pos;
	::Parser::str_t _0;
	::Parser::bool_t success_1 = false;
	if (!(!std::strncmp(pos, "num", 3)))
	{
		return {};
	}
	_0 += ::Parser::str_t(pos, 3);
	success_1 = true;
	pos += 3;
	AUTO_30_data data = _0;
	return {true, ::Parser::Token(in - str, in, pos, Tokens::AUTO_30, data)};
}
Parser::Token_res Parser::Tokenizator::AUTO_31(const char* &pos) {
	auto in = pos;
	::Parser::str_t _0;
	::Parser::bool_t success_1 = false;
	if (!(!std::strncmp(pos, "bool", 4)))
	{
		return {};
	}
	_0 += ::Parser::str_t(pos, 4);
	success_1 = true;
	pos += 4;
	AUTO_31_data data = _0;
	return {true, ::Parser::Token(in - str, in, pos, Tokens::AUTO_31, data)};
}
Parser::Token_res Parser::Tokenizator::AUTO_32(const char* &pos) {
	auto in = pos;
	::Parser::str_t _0;
	::Parser::bool_t success_1 = false;
	if (!(!std::strncmp(pos, "str", 3)))
	{
		return {};
	}
	_0 += ::Parser::str_t(pos, 3);
	success_1 = true;
	pos += 3;
	AUTO_32_data data = _0;
	return {true, ::Parser::Token(in - str, in, pos, Tokens::AUTO_32, data)};
}
Parser::Token_res Parser::Tokenizator::AUTO_33(const char* &pos) {
	auto in = pos;
	::Parser::str_t _0;
	::Parser::bool_t success_1 = false;
	if (!(!std::strncmp(pos, "arr", 3)))
	{
		return {};
	}
	_0 += ::Parser::str_t(pos, 3);
	success_1 = true;
	pos += 3;
	AUTO_33_data data = _0;
	return {true, ::Parser::Token(in - str, in, pos, Tokens::AUTO_33, data)};
}
Parser::Token_res Parser::Tokenizator::AUTO_34(const char* &pos) {
	auto in = pos;
	::Parser::str_t _0;
	::Parser::bool_t success_1 = false;
	if (!(!std::strncmp(pos, "obj", 3)))
	{
		return {};
	}
	_0 += ::Parser::str_t(pos, 3);
	success_1 = true;
	pos += 3;
	AUTO_34_data data = _0;
	return {true, ::Parser::Token(in - str, in, pos, Tokens::AUTO_34, data)};
}
Parser::Token_res Parser::Tokenizator::AUTO_35(const char* &pos) {
	auto in = pos;
	::Parser::arr_t<::Parser::str_t> shadow_2;
	::Parser::str_t _0;
	::Parser::bool_t success_1 = false;
	if (*(pos + 0) != ' ')
	{
		return {};
		shadow_2.push_back(_0);
	}
	_0 += ::Parser::str_t(pos, 1);
	success_1 = true;
	pos += 1;
	AUTO_35_data data = shadow_2;
	return {true, ::Parser::Token(in - str, in, pos, Tokens::AUTO_35, data)};
}
Parser::Token_res Parser::Tokenizator::AUTO_36(const char* &pos) {
	auto in = pos;
	::Parser::str_t _0;
	::Parser::bool_t success_1 = false;
	if (!(!std::strncmp(pos, "if", 2)))
	{
		return {};
	}
	_0 += ::Parser::str_t(pos, 2);
	success_1 = true;
	pos += 2;
	AUTO_36_data data = _0;
	return {true, ::Parser::Token(in - str, in, pos, Tokens::AUTO_36, data)};
}
Parser::Token_res Parser::Tokenizator::AUTO_48(const char* &pos) {
	auto in = pos;
	::Parser::str_t _0;
	::Parser::bool_t success_1 = false;
	if (!(!std::strncmp(pos, "fn", 2)))
	{
		return {};
	}
	_0 += ::Parser::str_t(pos, 2);
	success_1 = true;
	pos += 2;
	AUTO_48_data data = _0;
	return {true, ::Parser::Token(in - str, in, pos, Tokens::AUTO_48, data)};
}
Parser::Token_res Parser::Tokenizator::AUTO_49(const char* &pos) {
	auto in = pos;
	::Parser::str_t _0;
	::Parser::bool_t success_1 = false;
	if (!(!std::strncmp(pos, "while", 5)))
	{
		return {};
	}
	_0 += ::Parser::str_t(pos, 5);
	success_1 = true;
	pos += 5;
	AUTO_49_data data = _0;
	return {true, ::Parser::Token(in - str, in, pos, Tokens::AUTO_49, data)};
}
Parser::Token_res Parser::Tokenizator::AUTO_55(const char* &pos) {
	auto in = pos;
	::Parser::str_t _0;
	::Parser::bool_t success_1 = false;
	if (!(*(pos + 0) == '='))
	{
		return {};
	}
	_0 += ::Parser::str_t(pos, 1);
	success_1 = true;
	pos += 1;
	AUTO_55_data data = _0;
	return {true, ::Parser::Token(in - str, in, pos, Tokens::AUTO_55, data)};
}
Parser::Token_res Parser::Tokenizator::AUTO_60(const char* &pos) {
	auto in = pos;
	::Parser::str_t _0;
	::Parser::bool_t success_1 = false;
	if (!(!std::strncmp(pos, "++", 2)))
	{
		return {};
	}
	_0 += ::Parser::str_t(pos, 2);
	success_1 = true;
	pos += 2;
	AUTO_60_data data = _0;
	return {true, ::Parser::Token(in - str, in, pos, Tokens::AUTO_60, data)};
}
Parser::Token_res Parser::Tokenizator::AUTO_61(const char* &pos) {
	auto in = pos;
	::Parser::str_t _0;
	::Parser::bool_t success_1 = false;
	if (!(!std::strncmp(pos, "--", 2)))
	{
		return {};
	}
	_0 += ::Parser::str_t(pos, 2);
	success_1 = true;
	pos += 2;
	AUTO_61_data data = _0;
	return {true, ::Parser::Token(in - str, in, pos, Tokens::AUTO_61, data)};
}
Parser::Token_res Parser::Tokenizator::AUTO_65(const char* &pos) {
	auto in = pos;
	::Parser::str_t _0;
	::Parser::bool_t success_1 = false;
	if (!(*(pos + 0) == '{'))
	{
		return {};
	}
	_0 += ::Parser::str_t(pos, 1);
	success_1 = true;
	pos += 1;
	AUTO_65_data data = _0;
	return {true, ::Parser::Token(in - str, in, pos, Tokens::AUTO_65, data)};
}
Parser::Token_res Parser::Tokenizator::AUTO_70(const char* &pos) {
	auto in = pos;
	::Parser::str_t _0;
	::Parser::bool_t success_1 = false;
	if (!(*(pos + 0) == '}'))
	{
		return {};
	}
	_0 += ::Parser::str_t(pos, 1);
	success_1 = true;
	pos += 1;
	AUTO_70_data data = _0;
	return {true, ::Parser::Token(in - str, in, pos, Tokens::AUTO_70, data)};
}
Parser::Token_res Parser::Tokenizator::AUTO_71(const char* &pos) {
	auto in = pos;
	::Parser::str_t _0;
	::Parser::bool_t success_1 = false;
	if (!(!std::strncmp(pos, "spacemode", 9)))
	{
		return {};
	}
	_0 += ::Parser::str_t(pos, 9);
	success_1 = true;
	pos += 9;
	AUTO_71_data data = _0;
	return {true, ::Parser::Token(in - str, in, pos, Tokens::AUTO_71, data)};
}
Parser::Token_res Parser::Tokenizator::AUTO_72(const char* &pos) {
	auto in = pos;
	::Parser::str_t _0;
	::Parser::bool_t success_1 = false;
	if (!(!std::strncmp(pos, "mixed", 5)))
	{
		return {};
	}
	_0 += ::Parser::str_t(pos, 5);
	success_1 = true;
	pos += 5;
	AUTO_72_data data = _0;
	return {true, ::Parser::Token(in - str, in, pos, Tokens::AUTO_72, data)};
}
Parser::Token_res Parser::Tokenizator::AUTO_73(const char* &pos) {
	auto in = pos;
	::Parser::str_t _0;
	::Parser::bool_t success_1 = false;
	if (!(!std::strncmp(pos, "skipped", 7)))
	{
		return {};
	}
	_0 += ::Parser::str_t(pos, 7);
	success_1 = true;
	pos += 7;
	AUTO_73_data data = _0;
	return {true, ::Parser::Token(in - str, in, pos, Tokens::AUTO_73, data)};
}
Parser::Token_res Parser::Tokenizator::AUTO_74(const char* &pos) {
	auto in = pos;
	::Parser::str_t _0;
	::Parser::bool_t success_1 = false;
	if (!(!std::strncmp(pos, "allowed", 7)))
	{
		return {};
	}
	_0 += ::Parser::str_t(pos, 7);
	success_1 = true;
	pos += 7;
	AUTO_74_data data = _0;
	return {true, ::Parser::Token(in - str, in, pos, Tokens::AUTO_74, data)};
}
Parser::Token_res Parser::Tokenizator::AUTO_75(const char* &pos) {
	auto in = pos;
	::Parser::str_t _0;
	::Parser::bool_t success_1 = false;
	if (!(!std::strncmp(pos, "//", 2)))
	{
		return {};
	}
	_0 += ::Parser::str_t(pos, 2);
	success_1 = true;
	pos += 2;
	AUTO_75_data data = _0;
	return {true, ::Parser::Token(in - str, in, pos, Tokens::AUTO_75, data)};
}
Parser::Token_res Parser::Tokenizator::AUTO_76(const char* &pos) {
	auto in = pos;
	::Parser::str_t _0;
	::Parser::bool_t success_1 = false;
	if (!(!(*(pos + 0) == '\n')))
	{
		return {};
	}
	_0 += ::Parser::str_t(pos, 1);
	success_1 = true;
	pos += 1;
	AUTO_76_data data = _0;
	return {true, ::Parser::Token(in - str, in, pos, Tokens::AUTO_76, data)};
}
Parser::Token_res Parser::Tokenizator::AUTO_77(const char* &pos) {
	auto in = pos;
	::Parser::str_t _0;
	::Parser::bool_t success_1 = false;
	if (!(!std::strncmp(pos, "use", 3)))
	{
		return {};
	}
	_0 += ::Parser::str_t(pos, 3);
	success_1 = true;
	pos += 3;
	AUTO_77_data data = _0;
	return {true, ::Parser::Token(in - str, in, pos, Tokens::AUTO_77, data)};
}
Parser::Token_res Parser::Tokenizator::AUTO_78(const char* &pos) {
	auto in = pos;
	::Parser::str_t _0;
	::Parser::bool_t success_1 = false;
	if (!(*(pos + 0) == ','))
	{
		return {};
	}
	_0 += ::Parser::str_t(pos, 1);
	success_1 = true;
	pos += 1;
	AUTO_78_data data = _0;
	return {true, ::Parser::Token(in - str, in, pos, Tokens::AUTO_78, data)};
}
Parser::Token_res Parser::Tokenizator::AUTO_82(const char* &pos) {
	auto in = pos;
	::Parser::str_t _0;
	::Parser::bool_t success_1 = false;
	if (!(*(pos + 0) == '|'))
	{
		return {};
	}
	_0 += ::Parser::str_t(pos, 1);
	success_1 = true;
	pos += 1;
	AUTO_82_data data = _0;
	return {true, ::Parser::Token(in - str, in, pos, Tokens::AUTO_82, data)};
}
Parser::Token_res Parser::Tokenizator::AUTO_83(const char* &pos) {
	auto in = pos;
	::Parser::str_t _0;
	::Parser::bool_t success_1 = false;
	if (!(!std::strncmp(pos, "|&", 2)))
	{
		return {};
	}
	_0 += ::Parser::str_t(pos, 2);
	success_1 = true;
	pos += 2;
	AUTO_83_data data = _0;
	return {true, ::Parser::Token(in - str, in, pos, Tokens::AUTO_83, data)};
}
Parser::Token_res Parser::Tokenizator::AUTO_84(const char* &pos) {
	auto in = pos;
	::Parser::str_t _0;
	::Parser::bool_t success_1 = false;
	if (!(*(pos + 0) == '?'))
	{
		return {};
	}
	_0 += ::Parser::str_t(pos, 1);
	success_1 = true;
	pos += 1;
	AUTO_84_data data = _0;
	return {true, ::Parser::Token(in - str, in, pos, Tokens::AUTO_84, data)};
}
Parser::Token_res Parser::Tokenizator::AUTO_85(const char* &pos) {
	auto in = pos;
	::Parser::str_t _0;
	::Parser::bool_t success_1 = false;
	if (!(*(pos + 0) == '+'))
	{
		return {};
	}
	_0 += ::Parser::str_t(pos, 1);
	success_1 = true;
	pos += 1;
	AUTO_85_data data = _0;
	return {true, ::Parser::Token(in - str, in, pos, Tokens::AUTO_85, data)};
}
Parser::Token_res Parser::Tokenizator::AUTO_86(const char* &pos) {
	auto in = pos;
	::Parser::str_t _0;
	::Parser::bool_t success_1 = false;
	if (!(*(pos + 0) == '*'))
	{
		return {};
	}
	_0 += ::Parser::str_t(pos, 1);
	success_1 = true;
	pos += 1;
	AUTO_86_data data = _0;
	return {true, ::Parser::Token(in - str, in, pos, Tokens::AUTO_86, data)};
}
Parser::Token_res Parser::Tokenizator::AUTO_87(const char* &pos) {
	auto in = pos;
	::Parser::str_t _0;
	::Parser::bool_t success_1 = false;
	if (!(*(pos + 0) == '&'))
	{
		return {};
	}
	_0 += ::Parser::str_t(pos, 1);
	success_1 = true;
	pos += 1;
	AUTO_87_data data = _0;
	return {true, ::Parser::Token(in - str, in, pos, Tokens::AUTO_87, data)};
}
Parser::Token_res Parser::Tokenizator::AUTO_88(const char* &pos) {
	auto in = pos;
	::Parser::str_t _0;
	::Parser::bool_t success_1 = false;
	if (!(*(pos + 0) == '('))
	{
		return {};
	}
	_0 += ::Parser::str_t(pos, 1);
	success_1 = true;
	pos += 1;
	AUTO_88_data data = _0;
	return {true, ::Parser::Token(in - str, in, pos, Tokens::AUTO_88, data)};
}
Parser::Token_res Parser::Tokenizator::AUTO_89(const char* &pos) {
	auto in = pos;
	::Parser::str_t _0;
	::Parser::bool_t success_1 = false;
	if (!(*(pos + 0) == ')'))
	{
		return {};
	}
	_0 += ::Parser::str_t(pos, 1);
	success_1 = true;
	pos += 1;
	AUTO_89_data data = _0;
	return {true, ::Parser::Token(in - str, in, pos, Tokens::AUTO_89, data)};
}
Parser::Token_res Parser::Tokenizator::AUTO_90(const char* &pos) {
	auto in = pos;
	::Parser::str_t _0;
	::Parser::bool_t success_1 = false;
	if (!(*(pos + 0) == '['))
	{
		return {};
	}
	_0 += ::Parser::str_t(pos, 1);
	success_1 = true;
	pos += 1;
	AUTO_90_data data = _0;
	return {true, ::Parser::Token(in - str, in, pos, Tokens::AUTO_90, data)};
}
Parser::Token_res Parser::Tokenizator::AUTO_91(const char* &pos) {
	auto in = pos;
	::Parser::str_t _0;
	::Parser::bool_t success_1 = false;
	if (!(*(pos + 0) == '^'))
	{
		return {};
	}
	_0 += ::Parser::str_t(pos, 1);
	success_1 = true;
	pos += 1;
	AUTO_91_data data = _0;
	return {true, ::Parser::Token(in - str, in, pos, Tokens::AUTO_91, data)};
}
Parser::Token_res Parser::Tokenizator::AUTO_92(const char* &pos) {
	auto in = pos;
	::Parser::str_t _0;
	::Parser::bool_t success_1 = false;
	if (!(*(pos + 0) == ']'))
	{
		return {};
	}
	_0 += ::Parser::str_t(pos, 1);
	success_1 = true;
	pos += 1;
	AUTO_92_data data = _0;
	return {true, ::Parser::Token(in - str, in, pos, Tokens::AUTO_92, data)};
}
Parser::Token_res Parser::Tokenizator::AUTO_94(const char* &pos) {
	auto in = pos;
	::Parser::str_t _0;
	::Parser::bool_t success_1 = false;
	if (!(!std::strncmp(pos, "\\]", 2)))
	{
		return {};
	}
	_0 += ::Parser::str_t(pos, 2);
	success_1 = true;
	pos += 2;
	AUTO_94_data data = _0;
	return {true, ::Parser::Token(in - str, in, pos, Tokens::AUTO_94, data)};
}
Parser::Token_res Parser::Tokenizator::AUTO_95(const char* &pos) {
	auto in = pos;
	::Parser::str_t _0;
	::Parser::bool_t success_1 = false;
	if (!(!(*(pos + 0) == ']')))
	{
		return {};
	}
	_0 += ::Parser::str_t(pos, 1);
	success_1 = true;
	pos += 1;
	AUTO_95_data data = _0;
	return {true, ::Parser::Token(in - str, in, pos, Tokens::AUTO_95, data)};
}
Parser::Token_res Parser::Tokenizator::AUTO_98(const char* &pos) {
	auto in = pos;
	::Parser::str_t _0;
	::Parser::bool_t success_1 = false;
	if (!(*(pos + 0) == '-'))
	{
		return {};
	}
	_0 += ::Parser::str_t(pos, 1);
	success_1 = true;
	pos += 1;
	AUTO_98_data data = _0;
	return {true, ::Parser::Token(in - str, in, pos, Tokens::AUTO_98, data)};
}
Parser::Token_res Parser::Tokenizator::AUTO_99(const char* &pos) {
	auto in = pos;
	::Parser::str_t _0;
	::Parser::bool_t success_1 = false;
	if (!(*(pos + 0) == '.'))
	{
		return {};
	}
	_0 += ::Parser::str_t(pos, 1);
	success_1 = true;
	pos += 1;
	AUTO_99_data data = _0;
	return {true, ::Parser::Token(in - str, in, pos, Tokens::AUTO_99, data)};
}
Parser::Token_res Parser::Tokenizator::AUTO_101(const char* &pos) {
	auto in = pos;
	::Parser::str_t _0;
	::Parser::bool_t success_1 = false;
	if (!(!std::strncmp(pos, "data", 4)))
	{
		return {};
	}
	_0 += ::Parser::str_t(pos, 4);
	success_1 = true;
	pos += 4;
	AUTO_101_data data = _0;
	return {true, ::Parser::Token(in - str, in, pos, Tokens::AUTO_101, data)};
}
Parser::Token_res Parser::Tokenizator::AUTO_103(const char* &pos) {
	auto in = pos;
	::Parser::str_t _0;
	::Parser::bool_t success_1 = false;
	if (!(*(pos + 0) == ';'))
	{
		return {};
	}
	_0 += ::Parser::str_t(pos, 1);
	success_1 = true;
	pos += 1;
	AUTO_103_data data = _0;
	return {true, ::Parser::Token(in - str, in, pos, Tokens::AUTO_103, data)};
}
Parser::Token_res Parser::Tokenizator::AUTO_104(const char* &pos) {
	auto in = pos;
	::Parser::str_t _0;
	::Parser::bool_t success_1 = false;
	if (!(*(pos + 0) == '\n' || *(pos + 1) == '\r'))
	{
		return {};
	}
	_0 += ::Parser::str_t(pos, 2);
	success_1 = true;
	pos += 2;
	AUTO_104_data data = _0;
	return {true, ::Parser::Token(in - str, in, pos, Tokens::AUTO_104, data)};
}
Parser::Token_res Parser::Tokenizator::AUTO_105(const char* &pos) {
	auto in = pos;
	::Parser::str_t _0;
	::Parser::bool_t success_1 = false;
	if (!(*(pos + 0) == ':'))
	{
		return {};
	}
	_0 += ::Parser::str_t(pos, 1);
	success_1 = true;
	pos += 1;
	AUTO_105_data data = _0;
	return {true, ::Parser::Token(in - str, in, pos, Tokens::AUTO_105, data)};
}
Parser::Token_res Parser::Tokenizator::AUTO_106(const char* &pos) {
	auto in = pos;
	::Parser::str_t _0;
	::Parser::bool_t success_1 = false;
	if (!(!std::strncmp(pos, "\\", 1)))
	{
		return {};
	}
	_0 += ::Parser::str_t(pos, 1);
	success_1 = true;
	pos += 1;
	AUTO_106_data data = _0;
	return {true, ::Parser::Token(in - str, in, pos, Tokens::AUTO_106, data)};
}
Parser::Token_res Parser::Tokenizator::AUTO_107(const char* &pos) {
	auto in = pos;
	::Parser::str_t _0;
	::Parser::bool_t success_1 = false;
	if (*(pos + 0) == '\0')
	{
		return {};
	}
	_0 += ::Parser::str_t(pos, 1);
	success_1 = true;
	pos += 1;
	AUTO_107_data data = _0;
	return {true, ::Parser::Token(in - str, in, pos, Tokens::AUTO_107, data)};
}
Parser::Token_res Parser::Tokenizator::AUTO_108(const char* &pos) {
	auto in = pos;
	::Parser::str_t _0;
	::Parser::bool_t success_1 = false;
	if (!(*(pos + 0) == '\r' || *(pos + 1) == '\n'))
	{
		return {};
	}
	_0 += ::Parser::str_t(pos, 2);
	success_1 = true;
	pos += 2;
	AUTO_108_data data = _0;
	return {true, ::Parser::Token(in - str, in, pos, Tokens::AUTO_108, data)};
}
Parser::Token_res Parser::Tokenizator::AUTO_109(const char* &pos) {
	auto in = pos;
	::Parser::str_t _0;
	::Parser::bool_t success_1 = false;
	if (!(*(pos + 0) == '#'))
	{
		return {};
	}
	_0 += ::Parser::str_t(pos, 1);
	success_1 = true;
	pos += 1;
	AUTO_109_data data = _0;
	return {true, ::Parser::Token(in - str, in, pos, Tokens::AUTO_109, data)};
}
Parser::Token_res Parser::Tokenizator::AUTO_110(const char* &pos) {
	auto in = pos;
	::Parser::str_t _0;
	::Parser::bool_t success_1 = false;
	if (!(!std::strncmp(pos, "0x", 2)))
	{
		return {};
	}
	_0 += ::Parser::str_t(pos, 2);
	success_1 = true;
	pos += 2;
	AUTO_110_data data = _0;
	return {true, ::Parser::Token(in - str, in, pos, Tokens::AUTO_110, data)};
}
Parser::Token_res Parser::Tokenizator::AUTO_111(const char* &pos) {
	auto in = pos;
	::Parser::str_t _0;
	::Parser::bool_t success_1 = false;
	if (!((*(pos + 0)>='1' && *(pos + 0)<='9') || (*(pos + 1)>='A' && *(pos + 1)<='F') || (*(pos + 2)>='a' && *(pos + 2)<='f')))
	{
		return {};
	}
	_0 += ::Parser::str_t(pos, 3);
	success_1 = true;
	pos += 3;
	AUTO_111_data data = _0;
	return {true, ::Parser::Token(in - str, in, pos, Tokens::AUTO_111, data)};
}
Parser::Token_res Parser::Tokenizator::AUTO_112(const char* &pos) {
	auto in = pos;
	::Parser::str_t _0;
	::Parser::bool_t success_1 = false;
	if (!(!std::strncmp(pos, "0b", 2)))
	{
		return {};
	}
	_0 += ::Parser::str_t(pos, 2);
	success_1 = true;
	pos += 2;
	AUTO_112_data data = _0;
	return {true, ::Parser::Token(in - str, in, pos, Tokens::AUTO_112, data)};
}
Parser::Token_res Parser::Tokenizator::AUTO_113(const char* &pos) {
	auto in = pos;
	::Parser::str_t _0;
	::Parser::bool_t success_1 = false;
	if (!(*(pos + 0) == '0' || *(pos + 1) == '1'))
	{
		return {};
	}
	_0 += ::Parser::str_t(pos, 2);
	success_1 = true;
	pos += 2;
	AUTO_113_data data = _0;
	return {true, ::Parser::Token(in - str, in, pos, Tokens::AUTO_113, data)};
}
