			...
			if (!(!std::strncmp(pos, "str", 3)))
			{
				...
				if (!success_9)
				{
					...
					if (!(!std::strncmp(pos, "num", 3)))
					{
						...
						do
						{
							if (!(!std::strncmp(begin_23 + 0, "arr", 3)))
							{
								break;
							}
...
Parser::Token_res Parser::Lexer::rule_CSEQUENCE(const char* pos) {
	...
	{
		success_9 = false;
		_8 = rule_CSEQUENCE_ESCAPE(begin_17);
		if (!(_8.status))
		{
			success_12 = false;
			_11 = rule_CSEQUENCE_SYMBOL(begin_17);
			if (!(_11.status))
			{
				success_15 = false;
				_14 = rule_CSEQUENCE_DIAPASON(begin_17);
				if (!(_14.status))
				{
					break;
				}
